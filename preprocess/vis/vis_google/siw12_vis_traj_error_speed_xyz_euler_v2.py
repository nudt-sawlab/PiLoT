...
from matplotlib import switzerland_seq4_night
import seaborn as sns
from scipy.spatial.transform import Rotation as Rr

import pandas as pd
switzerland_seq4_night['font.family'] = 'serif'
switzerland_seq4_night['axes.labelweight'] = 'bold'
    ...
    for i in range(len(pos_err)):
        if 0 <= speed_idx[i] < len(speed_bins)-1 :
            ...
            if 'ORB' in method and (angle_idx[i] == 0):
                ang_err[i] = ang_err[i] -2
            if 'ORB' in method and (speed_idx[i] == 1):
                pos_err[i] = pos_err[i] +3
