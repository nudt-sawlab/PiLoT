...
def generate_checkerboard_overlay_transparent_v2(ref_crop, query_crop, rows, cols, alpha=0.7):
    ...
    return output
def generate_checkerboard_overlay_transparent(ref_crop, query_crop, rows, cols, alpha=0.9):
    """
    生成带透明度叠加的 checkerboard 图像。
...
def get_points_by_mouse(img, save_path=None, max_points=10,
    ...
    :param max_points: 最多点击数
    :param max_display_size: 显示窗口最大 (width, height)
    :return: numpy array shape=(N,2) in original image coords
    """
    orig_h, orig_w = img.shape[:2]
    disp_w, disp_h = max_display_size

    # 初始缩放比例让图 fit 到 max_display_size
    scale = min(disp_w / orig_w, disp_h / orig_h, 1.0)
    offset = np.array([0, 0], dtype=np.float32)  # 用于pan以后可扩展
    min_scale, max_scale = 0.2, 5.0

    clicked_points_display = []  # in display coords
    clicked_points_orig = []     # in original coords

    window_name = "Click Points (ESC to finish, Right=undo, Wheel=zoom)"

    def to_display(pt_orig):
        return ((pt_orig * scale) + offset).astype(int)

    def to_original(pt_disp):
        return ((np.array(pt_disp, dtype=np.float32) - offset) / scale)
    ...
    def redraw():
        ...
        # 缩放绘制
        disp = cv2.resize(img, None, fx=scale, fy=scale, interpolation=cv2.INTER_LINEAR)
        # 画已有点（display coords）
        for dp in clicked_points_display:
            cv2.circle(disp, tuple(int(x) for x in dp), 5, (0, 255, 255), -1, cv2.LINE_AA)
            cv2.circle(disp, tuple(int(x) for x in dp), 3, (0, 0, 0), 1, cv2.LINE_AA)
        current_img = disp
        cv2.imshow(window_name, disp)

    def mouse_cb(event, x, y, flags, param):
        nonlocal scale, offset
        if event == cv2.EVENT_LBUTTONDOWN and len(clicked_points_display) < max_points:
            # 记录 display 位置
            clicked_points_display.append((x, y))
            orig_pt = to_original((x, y))
            clicked_points_orig.append((orig_pt[0], orig_pt[1]))
            print(f"[Click] Point {len(clicked_points_orig)}: ({orig_pt[0]:.1f}, {orig_pt[1]:.1f})")
            redraw()
        elif event == cv2.EVENT_RBUTTONDOWN:
            # 撤销
            if clicked_points_display:
                clicked_points_display.pop()
                clicked_points_orig.pop()
                print(f"[Undo] 剩余点: {len(clicked_points_orig)}")
                redraw()
            ...
            # 为保持鼠标位置在原地，调整 offset：
            mx, my = x, y
            offset += np.array([mx, my]) - (np.array([mx, my]) * (scale / prev_scale))
            redraw()
    ...
    print(f"请在图像中点击 {max_points} 个点... (右键撤销，滚轮缩放)")

    while len(clicked_points_orig) < max_points:
        key = cv2.waitKey(20) & 0xFF
        if key == 27:  # ESC
            break

    cv2.destroyWindow(window_name)
    points = np.array(clicked_points_orig)

    if save_path:
        if save_path.endswith(".npy"):
            np.save(save_path, points)
        elif save_path.endswith(".txt"):
            np.savetxt(save_path, points, fmt='%.2f')
...
points_npy = "/mnt/sda/MapScape/query/estimation/mp4_compare/DJI_20250612194903_0021_V/vis_video_with_error/clicked_points.npy"
for method in methods:
    ref_dir = "/mnt/sda/MapScape/query/estimation/mp4_compare/DJI_20250612194903_0021_V/GT"
    query_dir = os.path.join("/mnt/sda/MapScape/query/estimation/mp4_compare/DJI_20250612194903_0021_V/", method)
    output_path = os.path.join("/mnt/sda/MapScape/query/estimation/mp4_compare/DJI_20250612194903_0021_V/vis_video_with_error/" , method)
    query_pose_file = os.path.join(query_dir, "DJI_20250612194903_0021_V.txt")
    ref_pose_file = os.path.join(ref_dir, "DJI_20250612194903_0021_V.txt")
    start_idx, end_idx = 333, 333
    ...
    for i in range(start_idx, end_idx + 1):
        ...
        qcamera = [1920, 1080, 1350.0, 1350.0, 957.85, 537.55]
        ...
        points2d_ref = points2d_ref  / 4
        points2d_query = points2d_query  / 2
