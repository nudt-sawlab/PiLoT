...
yaml_path = '/home/ubuntu/Documents/code/FPVLoc/src_open/preprocess/datasets/query/taiwan_seq1@200@30_50.yaml'
...
if __name__ == "__main__":
    ...
    # 3. 生成随机控制点构造轨迹
    # 控制点在局部 UTM 坐标下的 x 坐标均匀分布，从 0 到 FLIGHT_DISTANCE
    ...
    control_y = np.random.uniform(-LATERAL_MAX, LATERAL_MAX, NUM_CONTROL_POINTS)
    control_z = np.random.uniform(-VERTICAL_MAX, VERTICAL_MAX, NUM_CONTROL_POINTS)
    ...
    dz_dt = -cs_z.derivative()(t_vals)
    
    # 航向角 yaw：由水平速度方向决定（单位：度）
    tangent_angles = np.arctan2(dy_dt, dx_dt)
    yaw_degrees = (np.degrees(tangent_angles)-90) % 360
    # 调整使起始 yaw 与输入一致
    # yaw_adjust = INIT_EULER[2] - yaw_degrees[0]
    # yaw_degrees = (yaw_degrees + yaw_adjust) % 360
    
    # 俯仰角 pitch：由竖直分量与水平速度比值得到
    horizontal_speed = np.sqrt(dx_dt**2 + dy_dt**2)
    pitch_degrees = np.degrees(np.arctan2(dz_dt, horizontal_speed))
    # pitch_adjust = INIT_EULER[0] - pitch_degrees[0]
    # pitch_degrees = pitch_degrees + pitch_adjust
    
    # 滚转角 roll：在初始值基础上叠加随机扰动
    t = np.linspace(0,1,NUM_POINTS)
    roll_degrees = 0.3 * np.sin(2 * np.pi * t)#INIT_EULER[1] + np.random.normal(0, 5, NUM_POINTS)
