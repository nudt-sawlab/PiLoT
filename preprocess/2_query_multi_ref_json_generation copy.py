...
def generate_refer_info(sequence_folder, output_json, seq_name, ref_pose_dict , root):
    ...
    for pose_name, pose in tqdm(query_pose_dict.items()):
        ...
        lower_bound = max(0, query_index - 200)
        upper_bound = min(len(ref_pose_dict)-1, query_index + 200)
        ...
        if True:
            ...
            points2d_total, point3D_ref, origin_temp = get_3D_samples(points2d_ref, ref_depth_image, ref_T, qcamera)
            pose_query_origin = transform_ecef_origin(np.array(query_T), origin=origin)
            points2d_query_rej, points2d_ref_rej, Points_3D_ECEF_origin_rej, valid = get_points2D_ECEF_projection(pose_query_origin, qcamera, point3D_ref, points2d_total, num_samples=10000)
            
            points2d_query = points2d_query[valid]
            points2d_ref = points2d_ref[valid]
            Points_3D_ECEF_origin = Points_3D_ECEF_origin[valid]

            # 检查是否query像素坐标与反投影回query图像素坐标是否对不上
            max_distance = 2
            if points2d_query.shape != points2d_query_rej.shape:
                raise ValueError("Both point sets must have the same shape.")
            # 计算每对点之间的欧几里得距离
            distances = np.linalg.norm(points2d_query.cpu().numpy() - points2d_query_rej, axis=1)
            # 找到距离小于等于最大距离的索引
            num_samples = 5000
            valid_indices = np.where(distances <= max_distance)[0]
            # valid_indices_final = valid_indices
            step = len(valid_indices) / num_samples
            # 根据间隔采样索引
            valid_indices_final = [valid_indices[int(i * step)] for i in range(num_samples)]
            # num_points = 5000
            ...
            points2d_ref = points2d_ref[valid_indices_final]
            Points_3D_ECEF_origin = Points_3D_ECEF_origin[valid_indices_final]

            points2d_query_rej = points2d_query_rej[valid_indices_final]
            points2d_ref_rej = points2d_ref_rej[valid_indices_final]
    
            
            # for t in range(100):
            #     visualize_points_on_images(rgb_image, ref_image, 
            #                 [(points2d_query[t][0], points2d_query[t][1])], 
            #                 [(points2d_ref[t][0], points2d_ref[t][1])])
            #     visualize_points_on_images(rgb_image, ref_image, 
            #                 [(points2d_query_rej[t][0], points2d_query_rej[t][1])], 
            #                 [(points2d_ref_rej[t][0], points2d_ref_rej[t][1])])
            #     print(points2d_query[t],points2d_query_rej[t] )
            #     print('-')
