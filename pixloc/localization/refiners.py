...
class RetrievalRefiner(BaseRefiner):
    ...
    def refine(self, qname: str, qcamera: Camera, dbids: List[int] = None,
               loc: Optional[Dict] = None, Qt_init = None, p3did_to_dbids = None, last_frame_info = None) -> Dict:
        #! 4. set a Rt_init (array[3, 3], arrray([3, ]))
        if dbids is not None:
            ...
            T_init = Qt_init
        ret = self.refine_query_pose(qname, qcamera, T_init, p3did_to_dbids,
                                     self.conf.multiscale, last_frame_info = last_frame_info)
