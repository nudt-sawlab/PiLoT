...
class Interpolator:
    ...
    def __call__(self, tensor: torch.Tensor, pts: torch.Tensor,
        ...
        #     assert pts.dim() == 2
        #     batched = False
        #     tensor1, pts1 = tensor[None], pts[None]
        # else:
        #     batched = True
        #     tensor1, pts1 = tensor, pts

        # b, c, h, w = tensor1.shape
        # scale = torch.tensor([w-1, h-1]).to(pts1)
        # pts1 = (pts1 / scale) * 2 - 1
        # pts1 = pts1.clamp(min=-2, max=2)  # ideally use the mask instead
        # pts1 = pts1[:, 0, :].unsqueeze(1)
        # interpolated = torch.nn.functional.grid_sample(
        #         tensor1, pts1[:, None], mode='bilinear', align_corners=True)
        # interpolated = interpolated.reshape(b, c, -1).transpose(-1, -2)
