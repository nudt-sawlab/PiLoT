...
def rotation_matrix_to_angle_axis(R: torch.Tensor) -> torch.Tensor:
    """
    Convert batch of rotation matrices to angle-axis vectors.
    R: (N, 3, 3)
    Returns: (N, 3)
    """
    rotvec = torch.zeros(R.shape[0], 3, device=R.device, dtype=R.dtype)
    ...
    axis = torch.stack([R_diff[:, 2, 1], R_diff[:, 0, 2], R_diff[:, 1, 0]], dim=1) / 2

    rotvec[mask] = (theta[mask] / sin_theta[mask]).unsqueeze(-1) * axis[mask]
    return rotvec
...
class TensorWrapper:
    ...
    def squeeze(self, n):
        return self.__class__(self._data.squeeze(n))
    @classmethod
...
class Pose(TensorWrapper):
    ...
    def t(self) -> torch.Tensor:
        ...
        return self._data[..., -3:]
    def to_aa(self) -> torch.Tensor:
        """
        Convert SE(3) Pose to 6D angle-axis representation: [rx, ry, rz, tx, ty, tz]
        Returns:
            (N, 6) tensor
        """
        # 1. angle-axis from rotation matrix
        R = self.R  # (N, 3, 3)
        t = self.t  # (N, 3)

        rotvec = rotation_matrix_to_angle_axis(R)  # (N, 3)
        return torch.cat([rotvec, t], dim=-1)
    ...
    def compose(self, other: 'Pose') -> 'Pose':
        ...
        return self.__class__.from_Rt(R, t)
    def to_flat(self):
        R_flat = self.R.reshape(-1, 9)  # (N, 9)
        t_flat = self.t.reshape(-1, 3)  # (N, 3)
        return torch.cat([R_flat, t_flat], dim=-1)  # (N, 12)
...
class Camera(TensorWrapper):
    ...
    def crop(self, left_top: Tuple[float], size: Tuple[int]):
        ...
        return self.__class__(data)
    def to_tensor(self):
        return torch.cat([self.size, self.f, self.c])
