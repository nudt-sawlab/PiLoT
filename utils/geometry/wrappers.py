...
class TensorWrapper:
    ...
    def flatten(self, start_dim, end_dim):
        return self.__class__(self._data.flatten(start_dim, end_dim))
...
class Pose(TensorWrapper):
    ...
    def J_transform(self, p3d_out: torch.Tensor):
        ...
        # J_t = torch.zeros_like(J_rot, device=J_rot.device)
        # J_t[..., 0, 0] = torch.ones_like(J_t[..., 0, 0], device=J_rot.device)
        # J_t[..., 1, 1] = torch.ones_like(J_t[..., 1, 1], device=J_rot.device)
        # J_t[..., 2, 2] = torch.ones_like(J_t[..., 2, 2], device=J_rot.device)
