...
public:
    ModelRenderScene(std::string modelPath, uint32_t viewerWidth, uint32_t viewerHeight, double fovy, double aspectRatio);
    virtual ~ModelRenderScene();
    void shutdown()
    {
        _viewer->setDone(true);
        if (_viewer_thread.joinable())
            _viewer_thread.join();
    }
    osgViewer::Viewer *getViewer() { return _viewer.get(); }
    std::thread &getViewerThread() { return _viewer_thread; }
    // 渲染下一帧
    void nextFrame(double _fovy);
...
private:
    ...
    osg::ref_ptr<osgViewer::Viewer> createViewer(double fovy, double aspectRatio);
