...
read_pose_from_file() {
  local name="$1"
  local pose_file="/mnt/sda/MapScape/query/srt/${name}.txt"
...
for target_name in "${target_names[@]}"; do
  ...
  if [[ $index -ge 0 ]]; then
    ...
    echo "--- targetloc"
    python /home/ubuntu/Documents/code/github/FPV/FPV-Test-512-cuda/eval.py \
      --config "/home/ubuntu/Documents/code/github/FPV/FPV-Test-512-cuda/configs/feicuiwan_m4t.yaml" \
      --init_euler "$euler" \
      --init_trans "$trans" \
      --name "$target_name"
