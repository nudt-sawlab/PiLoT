...
class DualProcessTask:          # ← 名字随意
    ...
    def rendering_worker(self):
        ...
        while True:
            ...
            renderer.save_color_image(os.path.join(self.outputs, str(fps_log_every)+'.png'))
            ...
            # ---- 4) 推送给定位 ----
            try:
                self.task_q.put((color, P3d, T_w2c_mod,
                                T_init, dd, euler, trans), timeout=1)
            except queue.Full:
                break                       # 定位端不再消费，直接收尾
    ...
    def localization_worker(self):
        ...
        for idx, (img_path, img_tensor) in enumerate(zip(self.img_list, self.query_list)):
            ...
            if item is None:        # 渲染端提前喊停
                break
            color, P3d, T_w2c_mod, T_init, dd, render_euler, render_trans = item
            t0 = time.time()
            # 2) 调用定位
...
if __name__ == "__main__":
    ...
    config_file = '/home/ubuntu/Documents/code/github/FPV/FPV-Test-512-cuda/configs/feicuiwan@Unet_fusion.yaml'
