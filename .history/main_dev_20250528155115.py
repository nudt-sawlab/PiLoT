...
def load_poses(pose_file, origin):
    ...
    return pose_dict
def load_render_poses(pose_file, origin):
    """Load poses from the pose file."""
    pose_dict = []
    translation_list = []
    euler_angles_list = []
...
class DualThreadTask:
    def __init__(self, conf):
        ...
        # 
        # --- 渲染 / 定位 共享资源 ---
        self.task_q   = queue.Queue(maxsize=1)   # 渲染结果包
        self.pose_q   = queue.Queue(maxsize=1)   # 最新先验姿态
        ...
        self.render_pose_dict = load_render_poses(self.render_pose, origin = self.origin)
        # 定位初始化
    ...
    def rendering_worker(self):
        while not self.stop_evt.is_set():
            ...
            # 1) 渲染
            # for _ in range(1):
            #     self.renderer.update_pose(trans, euler, ref_camera=self.render_camera_osg)
            # color = self.renderer.get_color_image()
            # depth = self.renderer.get_depth_image()
            color, depth = self.render_list[self.num], self.render_depth_list[self.num]
            
            end_time = time.time()
            print('---render time: ', end_time - start_time)
            start_time = time.time()
            if self.padding:
                color = pad_to_multiple(color, padd=16)
            end_pad_time = time.time()
            print('---pad_time time: ', end_pad_time - start_time)
            start_time = time.time()
            # 2) 反投影 & 先验候选
            P3d, T_w2c_mod, T_init = self.back_project(depth, euler, trans)
            render_euler, render_trans = euler, trans
            end_time2 = time.time()
            print('---project time: ', end_time2 - start_time)
            # 3) 打包给定位线程
            self.task_q.put((color, P3d, T_w2c_mod, T_init, render_euler, render_trans))
    ...
    def localization_worker(self):
        ...
        for idx, (img_path, img_tensor) in enumerate(zip(self.img_list, self.query_list)):
            ...
            euler, trans = self.render_pose_dict[self.num][:3],self.render_pose_dict[self.num][3:]
            self.num+=1
            self.pose_q.put((euler, trans))
            ...
            end_time = time.time()
            print('Localization thread:', end_time - start_time)
    ...
    def run(self):
        t_render = threading.Thread(target=self.rendering_worker, daemon=True)
        t_local  = threading.Thread(target=self.localization_worker)

        t_render.start();  t_local.start()
        t_local.join();    t_render.join()
def parse_args():
...
if __name__ == "__main__":
    ...
    # # # # # # # # # # # # # 启动线程
    dual_thread_task.run()          # 内部会自己启动渲染/定位两个线程并阻塞到全部完成
