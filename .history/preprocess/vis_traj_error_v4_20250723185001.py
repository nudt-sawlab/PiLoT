...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        # ORB对齐
        if 'ORB' in method:
            scale, R, t = umeyama_alignment(rel[timestamps_est[:300]], rel[timestamps_est[:300]])
        ...
        large_pos = pos_err > 20
        large_ang = ang_err > 5

        # 方案三：误差热度图XY
        sc = ax_xy.scatter(rel[valid_mask, 0], rel[valid_mask, 1],
                           c=pos_err[valid_mask], cmap='jet',
                           s=10, label=method, alpha=0.8)

        # 高度曲线
        ax_alt.plot(full_timestamps[~large_pos], aligned_xyz[~large_pos, 2], color=color, linewidth=1.8)
        missing = np.isnan(aligned_xyz[:, 2])
        i = 0
        while i < len(full_timestamps):
            if missing[i] or large_pos[i]:
                start = i - 1
                while i < len(full_timestamps) and (missing[i] or large_pos[i]):
                    i += 1
                end = i if i < len(full_timestamps) else i - 1
                if 0 <= start < len(full_timestamps) and end < len(full_timestamps):
                    ax_alt.plot([full_timestamps[start], full_timestamps[end]],
                                [aligned_xyz[start, 2], aligned_xyz[end, 2]],
                                color=color, linestyle=(0, (10, 6)), linewidth=2.0,
                                alpha=0.8)
            else:
                i += 1
        ax_alt.scatter(full_timestamps[large_pos], aligned_xyz[large_pos, 2], color=color, s=10)

        # Pitch
        ax_pitch.plot(full_timestamps[~large_ang], aligned_angles[~large_ang, 0], color=color, linewidth=1.5)
        missing_p = np.isnan(aligned_angles[:, 0])
        i = 0
        while i < len(full_timestamps):
            if missing_p[i] or large_ang[i]:
                start = i - 1
                while i < len(full_timestamps) and (missing_p[i] or large_ang[i]):
                    i += 1
                end = i if i < len(full_timestamps) else i - 1
                if 0 <= start < len(full_timestamps) and end < len(full_timestamps):
                    ax_pitch.plot([full_timestamps[start], full_timestamps[end]],
                                  [aligned_angles[start, 0], aligned_angles[end, 0]],
                                  color=color, linestyle=(0, (10, 6)), linewidth=2.0,
                                  alpha=0.8)
            else:
                i += 1
        ax_pitch.scatter(full_timestamps[large_ang], aligned_angles[large_ang, 0], color=color, s=10)

        # Yaw
        ax_yaw.plot(full_timestamps[~large_ang], aligned_angles[~large_ang, 1], color=color, linewidth=1.5)
        missing_y = np.isnan(aligned_angles[:, 1])
        i = 0
        while i < len(full_timestamps):
            if missing_y[i] or large_ang[i]:
                start = i - 1
                while i < len(full_timestamps) and (missing_y[i] or large_ang[i]):
                    i += 1
                end = i if i < len(full_timestamps) else i - 1
                if 0 <= start < len(full_timestamps) and end < len(full_timestamps):
                    ax_yaw.plot([full_timestamps[start], full_timestamps[end]],
                                [aligned_angles[start, 1], aligned_angles[end, 1]],
                                color=color, linestyle=(0, (10, 6)), linewidth=2.0,
                                alpha=0.8)
            else:
                i += 1
        ax_yaw.scatter(full_timestamps[large_ang], aligned_angles[large_ang, 1], color=color, s=10)
    ...
    # 添加 colorbar
    fig.colorbar(sc, ax=ax_xy, label='Position Error (m)')

    ax_xy.set_title("Trajectory in XY Plane (Heatmap + Offset)")
    ...
    fig.tight_layout()
    fig.savefig(f"{outputs}/{seq_name}_heatmap_offset.png", dpi=300)
    plt.close(fig)

print("✅ 已完成方案一（轨迹偏移）+方案三（误差热度图）整合，输出至 outputs/")
