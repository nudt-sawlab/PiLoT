...
for seq in seq_list:
    ...
    fig, ax_yaw = plt.subplots(1, 1, figsize=(10, 6))
    ...
    for method, color in methods.items():
        ...
        valid_mask = ~np.isnan(aligned_angles[:, 1])

        ax_yaw.plot(full_timestamps[valid_mask], aligned_angles[valid_mask, 1], color=color, linewidth=2, label=method)
        ax_yaw.scatter(full_timestamps[~valid_mask], aligned_angles[~valid_mask, 1], color=color, s=10, alpha=0.6)

        all_yaw_data[method] = (full_timestamps, aligned_angles[:, 1], color)
    ...
    # ========== 动态放大镜（y轴极限放大） ==========
    axins = inset_axes(ax_yaw, width="28%", height="38%", loc='upper right', borderpad=2)
    ...
    def update_inset(x0):
        ...
        axins.clear()
        miny, maxy = None, None
        for method, (ts, yaw, color) in all_yaw_data.items():
            valid = ~np.isnan(yaw)
            axins.plot(ts[valid], yaw[valid], color=color, linewidth=2, label=method)
            in_window = (ts >= x0 - zoom) & (ts <= x0 + zoom) & valid
            if np.any(in_window):
                thismin, thismax = np.min(yaw[in_window]), np.max(yaw[in_window])
                if miny is None or thismin < miny:
                    miny = thismin
                if maxy is None or thismax > maxy:
                    maxy = thismax
        # y方向极限放大（窗口实际Yaw±margin）
        if miny is not None and maxy is not None:
            margin = (maxy - miny) * 0.3 + 0.02
            axins.set_ylim(miny - margin, maxy + margin)
        else:
            axins.set_ylim(ax_yaw.get_ylim())
    ...
    def on_key(event):
        if event.key.lower() == 's':
            save_path = f"{outputs}/{seq_name}_yaw_with_zoomed_magnifier.png"
            fig.canvas.draw()
            fig.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"✅ 按S已保存 Yaw主图+放大细节至 {save_path}")
...
print("✅ Yaw角随时间主图+动态极限细节放大镜完成，支持S键保存")
