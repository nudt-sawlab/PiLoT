...
def plot_with_virtual_lines(ax, x, y, color, large_error=None,label=None, linewidth=1.8,
    ...
    i = 0
    while i < len(y):
        if ( isnan[i]) or (large_error is not None and large_error[i]):
            start = i - 1
            while i < len(y) and (isnan[i] and (large_error is not None and large_error[i])):
                i += 1
            end = i
...
def plot_trajectory_with_gaps(ax, timestamps, traj, color, large_error=None, label=None, linewidth=2.2):
    ...
    i = 0
    while i < len(y):
        if isnan[i] or islarge[i]:
            start = i - 1
            while i < len(y) and (isnan[i] or islarge[i]):
                i += 1
            end = i
            if 0 <= start < len(y) and end < len(y):
                if (not isnan[start]) and (not isnan[end]) and \
                   (not islarge[start]) and (not islarge[end]):
                    ax.plot([x[start], x[end]], [y[start], y[end]],
                            color=color, linestyle=(0, (10, 6)), linewidth=2.0, alpha=0.8)
        else:
            i += 1
...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        plot_with_virtual_lines(ax_pitch, full_timestamps, aligned_angles[:, 0], color, large_error)
        ax_pitch.scatter(full_timestamps[large_error], aligned_angles[large_error, 0], color=color, s=10, alpha=0.6)

        plot_with_virtual_lines(ax_yaw, full_timestamps, aligned_angles[:, 1], color, large_error)
