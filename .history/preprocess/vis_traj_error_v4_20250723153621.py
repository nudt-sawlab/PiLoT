...
def plot_with_virtual_lines(ax, x, y, color, label=None, linewidth=1.8):
    isnan = np.isnan(y)
    valid = ~isnan
    ax.plot(x[valid], y[valid], color=color, linestyle='dashed',linewidth=linewidth, label=label)
    # for i in range(1, len(y) - 1):
    #     if isnan[i] and not isnan[i - 1] and not isnan[i + 1]:
    #         ax.plot([x[i - 1], x[i + 1]], [y[i - 1], y[i + 1]],
    #                 color=color, linestyle=(0, (6,6)), linewidth=1.2, alpha=0.6)
...
def plot_trajectory_with_gaps(ax, timestamps, traj, color, label=None, linewidth=2.2):
    ...
    for i in range(1, len(x) - 1):
        if isnan[i] and not isnan[i - 1] and not isnan[i + 1]:
            ax.plot([x[i - 1], x[i + 1]], [y[i - 1], y[i + 1]],
                    color=color, linestyle='dashed', linewidth=1.2, alpha=0.6)
...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        plot_with_virtual_lines(ax_alt, full_timestamps, aligned_xyz[:, 2], color)
