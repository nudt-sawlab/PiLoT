...
def plot_trajectory_with_gaps(ax, timestamps, traj, color, large_error=None, label=None, linewidth=2.2):
    ...
    for i, seg in enumerate(segments):
        ax.plot(seg[:, 0], seg[:, 1], color=color, linewidth=linewidth)
    # 添加虚线连接缺失段首尾
    i = 0
    while i < len(y):
        if isnan[i]:
                ...
                if (not isnan[start]) and (not isnan[end]) and \
                   (not islarge[start]) and (not islarge[end]):
                    ax.plot([x[start], x[end]], [y[start], y[end]],
                            color=color, linestyle='dashed', linewidth=2.0, alpha=0.8, label=method)
        else:
            i += 1
...
for seq in seq_list:
    ...
    fig, ax_xy = plt.subplots(1, 1, figsize=(10, 8))   # 只保留xy轨迹子图
    ...
    for method, color in methods.items():
        ...
        plot_trajectory_with_gaps(ax_xy, full_timestamps, rel, color, large_error)
        trajs_for_magnifier[method] = rel
