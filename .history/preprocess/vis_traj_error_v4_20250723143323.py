...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        # XY 平面误差带
        dx = poses_est[:, 0] - poses_gt[timestamps_est][:, 0]
        dy = poses_est[:, 1] - poses_gt[timestamps_est][:, 1]
        r = np.sqrt(dx ** 2 + dy ** 2)
        ...
        # 海拔、角度添加误差带
        plot_with_shaded_error(ax_alt, full_timestamps, poses_est[:, 2], np.abs(poses_est[:, 2] - poses_gt[timestamps_est][:, 2]), method, color)
        plot_with_shaded_error(ax_pitch, full_timestamps, angles_est[:, 0], np.abs(angles_est[:, 0] - angles_gt[timestamps_est][:, 0]), method, color)
        plot_with_shaded_error(ax_yaw, full_timestamps, angles_est[:, 1], np.abs(angles_est[:, 1] - angles_gt[timestamps_est][:, 1]), method, color)
