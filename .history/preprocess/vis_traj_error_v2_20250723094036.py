...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        # 3. Pitch 原始值
        ax_pitch.plot(timestamps, angles_est[:, 0], label=method, color=color, linewidth=1.5)

        # 4. Yaw 原始值
        ax_yaw.plot(timestamps, angles_est[:, 1], label=method, color=color, linewidth=1.5)
