...
for method in methods_label:
    ...
    for i in range(1, N):
        if np.any(np.isnan(est_xyz[i])) or np.any(np.isnan(gt_xyz[i])):
            continue
        # 平移误差（米）
        pe = np.linalg.norm(est_xyz[i] - gt_xyz[i])
        pos_err.append(pe)

        # 角度误差（deg）：由相机在 ECEF 下的 R_c2w 做相对旋转角
        # 对应帧的经纬高来自 GT，避免混用不同坐标
        llh_i = gt_llh_list[i]
        R_est = euler_angles_to_matrix_ECEF_c2w(est_angles[i], llh_i) if not np.any(np.isnan(est_angles[i])) else None
        R_gt  = euler_angles_to_matrix_ECEF_c2w(gt_angles[i],  llh_i)
        if R_est is None:
            continue
        cosang = np.clip((np.trace(R_gt.T @ R_est) - 1.0) * 0.5, -1.0, 1.0)
        ae = np.degrees(np.arccos(cosang))
        ang_err.append(ae)

        # IoU 桶（pair i-1 对应 i-1→i）
        b = pair_to_bin.get(i-1, None)
        bins_for_this_method.append(b)
