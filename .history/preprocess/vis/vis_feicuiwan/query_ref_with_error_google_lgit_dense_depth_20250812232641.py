...
for method in methods:
    ...
    for i in range(start_idx, end_idx + 1):
        ...
        err_px_map, valid_mask = dense_pixel_error_map_from_ref_depth(
            ref_depth=ref_depth,
            ref_T=ref_T,
            rcamera=rcamera,
            qry_T_gt=ref_T @ np.linalg.inv(ref_T) if False else qry_T,  # 这里放“GT Query 位姿”
            qry_T_est=query_T,                                          # 这里放“估计的 Query 位姿”
            qcamera=qcamera,
            stride=1,  # 2/3 可加速
        )

        # 归一化用于着色（不改变“像素单位”的语义）
        # 建议按像素阈值关心绝对误差，所以可视化用百分位归一化即可
        err_px_norm = normalize01(err_px_map, mask=valid_mask, p_low=1, p_high=99)

        # 热力图 + 等值线（暖色=误差大）
        heat_err = heatmap_with_contours(err_px_norm, valid_mask)

        # 二值高亮（像素阈值，例如 >2 px）
        THRESH_PX = 2.0
        mask_bin = (err_px_map >= THRESH_PX).astype(np.float32)
        red_highlight = overlay_binary_mask(cv2.resize(qry_img, (qcamera[0], qcamera[1])),
                                            mask_bin, color=(0,0,255), alpha=0.5)

        # 保存
        base = os.path.join(output_path, f"{i}_{method}")
        cv2.imwrite(base + "_errpx_heat.png", heat_err)
