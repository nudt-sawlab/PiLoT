...
def draw_curved_arrow_with_endpoints(img, pt1, pt2, color, thickness=2, arrow_size=6):
    ...
    # 画圆圈：起点红圈，终点绿圈
    cv2.circle(img, tuple(np.round(pt1).astype(int)), 4, (0, 0, 255), 1, cv2.LINE_AA)  # Red circle
    cv2.circle(img, tuple(np.round(pt2).astype(int)), 4, (0, 255, 0), 1, cv2.LINE_AA)  # Green circle
...
#----
    ...
    for (x1, y1), (x2, y2), err_norm in zip(pts_query_visible, pts_ref_visible, norm_err):
        ...
        draw_curved_arrow_with_endpoints(frame, pt1, pt2, color, thickness=1)
    # 保存结果图（无 colorbar）
