...
#     return output
def draw_curved_arrow_with_endpoints_v2(img, pt1, pt2, color, thickness=2, arrow_size=6):
    ...
    # === 强制拉长箭头 ===
    FORCE_LEN = 30  # 可调：弯出来的视觉长度
    unit_vec = vec / length
    pt2 = pt1 + unit_vec * FORCE_LEN  # 无论多近都拉出 30px 方向箭头

    # === 控制弯曲程度 ===
    ortho = np.array([-unit_vec[1], unit_vec[0]])  # 垂直方向
    bend_strength = 0.8  # 可调：越大越弯（建议 0.5 ~ 1.0）
    ctrl = pt1 + 0.5 * (pt2 - pt1) + ortho * FORCE_LEN * bend_strength
...
#----
    ...
    for (x1, y1), (x2, y2), err_norm in zip(pts_query_visible, pts_ref_visible, norm_err):
        ...
        draw_curved_arrow_with_endpoints_v2(frame, pt1, pt2, color, thickness=1)
    # 保存结果图（无 colorbar）
