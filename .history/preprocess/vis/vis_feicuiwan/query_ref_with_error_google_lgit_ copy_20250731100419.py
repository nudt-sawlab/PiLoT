...
#----
    ...
    # === 使用像素误差 ===
    errors = np.linalg.norm(pts_query_visible - pts_ref_visible, axis=1)

    # 映射到 jet colormap：颜色表示 Pixel Error
    vmin, vmax = 0.0, 10.0  # 最大误差为10像素
    norm_err = np.clip((errors - vmin) / (vmax - vmin), 0.0, 1.0)
    ...
    # === 保存图像（Pixel Error 可视化） ===
    cv2.imwrite(f'{output_path}/{i}_arrow_pixel_error.png', frame)
