...
for method in methods:
    ...
    output_path = os.path.join("/mnt/sda/MapScape/query/estimation/mp4_compare/USA_seq5_foggy_gif/vis_video_with_error/" , method)
    ...
    #----
        ...
        # 假设 errors_3d_visible 最小大于0
        # 1) 构造 LogNorm 和 ScalarMappable
        norm = LogNorm(
            vmin=max(errors_3d_visible.min(), 1e-3),
            vmax=errors_3d_visible.max()
        )
        sm = plt.cm.ScalarMappable(norm=norm, cmap='viridis')
        sm.set_array([])

        # 2) 画一次 colorbar（在所有帧画完后，或者每个子图里只调一次）
        plt.colorbar(sm, label='3D error (m)')

        # 3) 循环里取原始误差 values，而不是 norm_err
        for (x1, y1), (x2, y2), err in zip(pts_query_visible, pts_ref_visible, errors_3d_visible):
            pt1 = (int(round(x1)), int(round(y1)))
            pt2 = (int(round(x2)), int(round(y2)))

            # 4) 用 LogNorm 把真实误差映射到 [0,1] RGBA
            rgba = sm.to_rgba(err)               # e.g. (r,g,b,a) 都是 0~1
            # 转成 OpenCV 需要的 BGR 整数
            bgr = (
                int(rgba[2] * 255),
                int(rgba[1] * 255),
                int(rgba[0] * 255)
            ...
            # 5) 调用箭头函数，传入 bgr tuple
            draw_variable_curve_arrow(frame, pt1, pt2, color=bgr, thickness=2)
        # 保存结果图（无 colorbar）
