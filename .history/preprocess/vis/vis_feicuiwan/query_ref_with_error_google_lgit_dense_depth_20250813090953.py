...
对的。为了公平对比多算法/多帧，不要做每帧归一化；用统一的像素范围映射成热力图，并且等值线用固定的像素阈值。下面给你一段替换可视化部分的补丁：固定上限 PIX_VMAX（比如 5px 或 10px），固定等值线 CONTOUR_LEVELS_PX（如 [1,2,3,5]）。
...
for method in methods:
    ...
    for i in range(start_idx, end_idx + 1):
        ...
        # 可视化：模长热力图 + 等值线
        # err_mag_norm = normalize01(err_mag, mask=valid)
        heat_mag = heatmap_with_contours(err_mag, valid)
        base = os.path.join(output_path, f"{i}_{method}")
        ...
        THRESH_PX = 2.0
        mask_bin = (err_mag >= THRESH_PX).astype(np.float32)
        red = overlay_binary_mask(cv2.resize(qry_img, (qcamera[0], qcamera[1])), mask_bin, (0,0,255), 0.5)
        cv2.imwrite(base + f"_err_gt{THRESH_PX:.1f}px.png", red)

        # 如果想看水平/垂直分量（可选）
        err_u_norm = normalize01(np.abs(err_u), mask=valid)
        err_v_norm = normalize01(np.abs(err_v), mask=valid)
        cv2.imwrite(base + "_err_u_heat.png", heatmap_with_contours(err_u_norm, valid))
        cv2.imwrite(base + "_err_v_heat.png", heatmap_with_contours(err_v_norm, valid))
        # cv2.imwrite(base + f"_errpx_gt{THRESH_PX:.1f}px.png", red_highlight)
