...
def draw_error_points_on_image_v2(image, xy_dict, error_t,
                               point_radius=6, thickness=-1,
                               colormap=cv2.COLORMAP_JET):
    """
    误差大的点画在上层；inf（missing）用空心灰表示；无 error_t 时标注提示。
    """
    img = image.copy()
    ...
    norm_err = normalize_error_dict(finite_errors)  # 只有 finite keys

    # 为了把大误差画在上面，按原始 error 降序排序（inf 放最前面）
    ...
    # 如果有太多 missing_in_xy, 只展示前几个
    if missing_in_xy:
        sample = missing_in_xy[:5]
        txt = "Missing XY for: " + ",".join(sample)
        cv2.putText(img, txt, (10, img.shape[0]-10), font, 0.4, (0, 255, 255), 1, cv2.LINE_AA)
        if len(missing_in_xy) > 5:
            more = f"+{len(missing_in_xy)-5} more"
            cv2.putText(img, more, (10, img.shape[0]-30), font, 0.4, (0, 255, 255), 1, cv2.LINE_AA)
    ...
    # 标注 error 范围（用 finite errors 做 percentile）
    all_errors = np.array(list(finite_errors.values()), dtype=np.float32)
    if all_errors.size > 0:
        ...
        cv2.putText(img, "No finite error", (bar_x - 5, bar_y + bar_h//2), font, 0.5, (0,0,255), 1, cv2.LINE_AA)
