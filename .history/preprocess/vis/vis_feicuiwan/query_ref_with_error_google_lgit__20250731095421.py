...
#----
    ...
    # 判断两个点是否都在图像范围内
    in_ref = (pts_ref_scaled[:, 0] >= 0) & (pts_ref_scaled[:, 0] < w_img) & \
            (pts_ref_scaled[:, 1] >= 0) & (pts_ref_scaled[:, 1] < h_img)

    in_qry = (pts_query_scaled[:, 0] >= 0) & (pts_query_scaled[:, 0] < w_img) & \
            (pts_query_scaled[:, 1] >= 0) & (pts_query_scaled[:, 1] < h_img)
    ...
    # 归一化误差用于 colormap
    errors = np.linalg.norm(pts_query_scaled - pts_ref_scaled, axis=1)
    norm_errors = (errors - errors.min()) / (errors.ptp() + 1e-5)
    cmap = cm.get_cmap('jet')

    for (x1, y1), (x2, y2), err_norm in zip(pts_ref_scaled, pts_query_scaled, norm_errors):
        pt1 = tuple(np.round([x1, y1]).astype(int))
        pt2 = tuple(np.round([x2, y2]).astype(int))
        
        # 使用颜色映射误差值
        color = tuple(int(255 * c) for c in cmap(err_norm)[:3][::-1])  # RGB to BGR

        # 画箭头
        cv2.arrowedLine(frame, pt1, pt2, color, 2, tipLength=0.25)
    cv2.imwrite(f'{output_path}/{i}_0.png', frame)
    # cv2.imwrite(f'{output_path}/{i}_0.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 90])
    print(f'{output_path}/{i}_0.png')
    print('-')
