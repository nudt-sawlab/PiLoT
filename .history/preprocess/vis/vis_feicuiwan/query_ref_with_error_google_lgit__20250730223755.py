...
query_dir = "/mnt/sda/MapScape/query/estimation/mp4_compare/USA_seq5_foggy_gif/Render2Loc"
output_path = "/mnt/sda/MapScape/query/estimation/mp4_compare/USA_seq5_foggy_gif/vis_video_with_error/Render2Loc"
...
#---
    ...
    qcamera = np.array([1920, 1080, 2317.6, 2317.6, 960.0, 540.0]) / 2
    ey = np.random.randint(0, H, size= 50)
    ex = np.random.randint(0, W, size= 50)
    points2d_query = np.column_stack((ex, ey)) 
    query_depth_image = np.load(query_path[:-4]+'.npy')
    ref_T = gt_angles[np.where(gt_timestamps == i)][0]
    query_T = es_angles[np.where(es_timestamps == i)][0]
    ref_T[:3, 1] = -ref_T[:3, 1]  # Y 轴取反
    ref_T[:3, 2] = -ref_T[:3, 2]  # Z 轴取反
    
    query_T[:3, 1] = -query_T[:3, 1]  # Y 轴取反
    query_T[:3, 2] = -query_T[:3, 2]  # Z 轴取反
    points2d_ref_valid, point3D_from_ref, _, _ = get_3D_samples_v2(points2d_query, query_depth_image, ref_T, rcamera)
    points2d_query, _, Points_3D_ECEF_origin, valid = get_points2D_ECEF_projection(np.array(query_T), qcamera, point3D_from_ref, points2d_ref_valid, use_valid = False)   
