...
#----
    ...
    if 'points2d_ref' in locals() and 'points2d_query' in locals():
        # 可选缩放（你有 /2 操作，确认是否一致）
        ...
        # 如果做了 crop，需要减去 crop offset
        pts_ref_scaled -= np.array([x_off * scale_x, y_off * scale_y])
        pts_query_scaled -= np.array([x_off * scale_x, y_off * scale_y])

        for p_ref, p_qry in zip(pts_ref_scaled, pts_query_scaled):
            x1, y1 = int(p_ref[0]), int(p_ref[1])
            x2, y2 = int(p_qry[0]), int(p_qry[1])

            # 画点
            cv2.circle(frame, (x1, y1), 3, (0, 255, 0), -1)  # 绿色：ref 投影
            cv2.circle(frame, (x2, y2), 3, (0, 0, 255), -1)  # 红色：query 投影

            # 连线显示距离差
            cv2.line(frame, (x1, y1), (x2, y2), (255, 0, 0), 1)  # 蓝线：ref → query
