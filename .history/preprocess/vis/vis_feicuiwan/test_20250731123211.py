...
def draw_curved_arrow(img, pt1, pt2, color=(0, 255, 0), thickness=2, arrow_size=8, bend_scale=0.25, extend_ratio=0.2):
    """
    绘制从 pt1 到 pt2 的一段平滑弯曲箭头。
    即使距离很近，也会绘制较明显的弯曲。
    - pt1, pt2: 起点终点 (x, y)
    - bend_scale: 控制弯曲强度的比例
    - extend_ratio: 控制起终点向外延伸的比例（增强视觉曲率）
    """
    ...
    # 延伸起点与终点
    pt1_ext = pt1 - dir_unit * length * extend_ratio
    pt2_ext = pt2 + dir_unit * length * extend_ratio

    # 控制点：中点向法向偏移
    midpoint = (pt1_ext + pt2_ext) / 2
    bend = max(bend_scale * length, 20)  # 小距离时弯曲不能太小
    ctrl = midpoint + ortho * bend

    # 二次贝塞尔插值（pt1_ext → ctrl → pt2_ext）
    curve = []
...
draw_curved_arrow(img, (150, 200), (160, 210))  # 距离很近
