...
def draw_error_points_on_image(image, xy_dict, error_t,
    ...
    cv2.rectangle(img, (bar_x, bar_y), (bar_x + bar_w, bar_y + bar_h), (220, 220, 220), 1, cv2.LINE_AA)

    # 标题和说明
    cv2.putText(img, "Error", (bar_x, bar_y - 20), font, 0.55, (240, 240, 240), 1, cv2.LINE_AA)
    cv2.putText(img, "clipped to [0,5]m", (bar_x, bar_y - 6), font, 0.35, (180, 180, 180), 1, cv2.LINE_AA)
    ...
    for v in major_ticks:
        y_pos = val_to_y(v)
        cv2.line(img, (bar_x + bar_w + 2, y_pos), (bar_x + bar_w + 8, y_pos), (220, 220, 220), 1, cv2.LINE_AA)
        label = f"{v:.1f}m"
        txt_w, txt_h = cv2.getTextSize(label, font, 0.4, 1)[0]
        cv2.putText(img, label, (bar_x + bar_w + 10, y_pos + txt_h // 2), font, 0.4, (240, 240, 240), 1, cv2.LINE_AA)

    # 次刻度（如 4,3,1 米）
    for v in [4.0, 3.0, 1.0]:
        y_pos = val_to_y(v)
        cv2.line(img, (bar_x + bar_w + 2, y_pos), (bar_x + bar_w + 6, y_pos), (180, 180, 180), 1, cv2.LINE_AA)
