...
def generate_full_checkerboard(ref_img, query_img, rows, cols):
    ...
    return output
def generate_checkerboard_overlay_transparent_v2(ref_crop, query_crop, rows, cols, alpha=0.95):
    """
    ...
    return output
def generate_checkerboard_overlay_transparent(ref_crop, query_crop, rows, cols, alpha=0.92):
    """
    生成带透明度叠加的 checkerboard 图像。
...
#----
    ...
    # 设置映射范围为 0~10 米
    vmin, vmax = 0.0, 100.0
    norm_err = np.clip((errors_3d_visible - vmin) / (vmax - vmin), 0.0, 1.0)
    cmap = cm.get_cmap('jet')
    arrow_thickness = 2  # 更细
    ...
    # 保存结果图（无 colorbar）
    cv2.imwrite(f'{output_path}/{i}_0.png', frame)
    print(f'{output_path}/{i}_arrow3d.png')
