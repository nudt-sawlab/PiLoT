...
def draw_error_points_on_image(image, xy_dict, error_t,
    ...
    for v in major_ticks:
        y_pos = val_to_y(v)
        # 长刻度线
        cv2.line(img, (bar_x + bar_w + 2), y_pos, (bar_x + bar_w + 15), y_pos, (220, 220, 220), 2)
        label = f"{v:.1f}m"
        txt_w, txt_h = cv2.getTextSize(label, font, 0.45, 1)[0]
        label_x = bar_x + bar_w + 17
        label_y = y_pos + txt_h // 2
        # 背景块提高清晰度（可选）
        cv2.rectangle(img, (label_x - 2, label_y - txt_h), (label_x + txt_w + 2, label_y + 2), (30, 30, 30), -1)
        cv2.putText(img, label, (label_x, label_y), font, 0.45, (240, 240, 240), 1)
