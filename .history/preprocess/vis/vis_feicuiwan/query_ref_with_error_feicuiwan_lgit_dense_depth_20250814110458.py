...
def dense_pixel_error_xy_from_ref_depth(ref_depth, ref_T, rcamera,
    ...
    Xc = np.stack([(uu - cx_r) / fx_r * z,
                   (vv - cy_r) / fy_r * z,
                   z], axis=0).astype(np.float64)  # (3,N)

    # 公共：世界坐标
    Rr = ref_T[:3, :3].astype(np.float64)
    tr = ref_T[:3, 3].astype(np.float64).reshape(3,1)
    a = Rr @ Xc
    Xw = Rr @ Xc + tr
...
for method in methods:
    ref_dir   = "/mnt/sda/MapScape/query/estimation/mp4_compare/DJI_20250612174308_0001_V/GT"         # Render/Ref
    query_dir = os.path.join("/mnt/sda/MapScape/query/estimation/mp4_compare/DJI_20250612174308_0001_V/", method)  # Query
    output_path = os.path.join("/mnt/sda/MapScape/query/estimation/mp4_compare/DJI_20250612174308_0001_V/vis/pix_error_heat_vis")
