...
def draw_arc_through_points(img, pt1, pt2, color=(0, 255, 0), thickness=2, 
    ...
    # 起点对齐（补 pt1）+ 画圆弧
    cv2.line(img, tuple(pt1.astype(int)), arc_pts[0], color, thickness, cv2.LINE_AA)
    for i in range(len(arc_pts) - 1):
        cv2.line(img, tuple(arc_pts[i]), tuple(arc_pts[i + 1]), color, thickness, cv2.LINE_AA)
    cv2.line(img, arc_pts[-1], tuple(pt2.astype(int)), color, thickness, cv2.LINE_AA)
...
draw_arc_through_points(img, (200, 200), (210, 210), color=(0, 0, 255), arc_radius=100)
draw_arc_through_points(img, (100, 100), (300, 150), color=(0, 128, 0), arc_radius=200)
draw_arc_through_points(img, (100, 400), (700, 450), color=(255, 0, 0), arc_radius=250, arc_direction='right')

cv2.imshow("Fixed Arc Arrows", img)
cv2.waitKey(0)
