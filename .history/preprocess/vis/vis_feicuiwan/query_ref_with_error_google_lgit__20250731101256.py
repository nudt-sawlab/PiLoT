...
query_dir = "/mnt/sda/MapScape/query/estimation/mp4_compare/USA_seq5_foggy_gif/Render2RAFT"
output_path = "/mnt/sda/MapScape/query/estimation/mp4_compare/USA_seq5_foggy_gif/vis_video_with_error/Render2RAFT"
...
#---
    ...
    point3D_query_valid = point3D_from_ref_like_query[valid]
...
#----
    ...
    errors_3d_visible = errors_3d[valid_mask]  # 匹配误差长度

    # colormap 映射
    norm_err = (errors_3d_visible - errors_3d_visible.min()) / (errors_3d_visible.ptp() + 1e-5)
    colors = cm.jet(norm_err)

    # Matplotlib 可视化
    ref_vis = cv2.cvtColor(frame.copy(), cv2.COLOR_BGR2RGB)
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.imshow(ref_vis)

    for (x1, y1), (x2, y2), color in zip(pts_query_visible, pts_ref_visible, colors):
        dx = x2 - x1
        dy = y2 - y1
        ax.arrow(x1, y1, dx, dy, color=color, width=0.5, head_width=4, length_includes_head=True)
        ax.plot(x1, y1, 'rx', markersize=5)
        ax.plot(x2, y2, 'go', markersize=5)

    # 添加 colorbar
    sm = plt.cm.ScalarMappable(cmap='jet', norm=plt.Normalize(vmin=errors_3d_visible.min(), vmax=errors_3d_visible.max()))
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax)
    cbar.set_label("3D Distance Error (m)", fontsize=12)

    ax.set_xticks([])
    ax.set_yticks([])
    plt.tight_layout()
    plt.savefig(f"{output_path}/{i}_3d_arrow_error.jpg", dpi=200)
    plt.close()
