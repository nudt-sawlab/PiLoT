...
def draw_error_points_on_image(image, xy_dict, error_t,
    ...
    """
    更美化版：底图黑白、colorbar 做刻度/边框/标签、误差大的在顶层、inf 用空心标。
    """
    # 先做黑白底图
    img = to_grayscale_bg(image)

    font = cv2.FONT_HERSHEY_DUPLEX  # 比较干净的字体
    ...
    finite_errors = {k: v for k, v in error_t.items() if np.isfinite(v)}
    norm_err = normalize_error_dict(finite_errors)
    ...
    for key in sorted_keys:
        ...
        if not np.isfinite(error_t[key]):
            # inf 用空心灰圈 + 问号
            cv2.circle(img, (int(x), int(y)), point_radius, (180, 180, 180), 2, lineType=cv2.LINE_AA)
        ...
        # 加一个轻微的外发光（先画一个较粗半透明底，再画实心）
        glow_radius = point_radius + 2
        overlay = img.copy()
        cv2.circle(overlay, (int(x), int(y)), glow_radius, color, -1, lineType=cv2.LINE_AA)
        alpha = 0.3
    ...
    if missing_in_xy:
        ...
        cv2.rectangle(img, (5, img.shape[0]-60), (img.shape[1]//2, img.shape[0]-5), (0,0,0), -1)  # 半透明背景
    ...
    colored_bar = cv2.applyColorMap(gradient, colormap)  # 用 Turbo 映射

    # 放一个略带圆角的底板（浅半透明）增强对比
    pad = 6
    bg_x0 = bar_x - pad
    bg_y0 = bar_y - pad
    bg_x1 = bar_x + bar_w + pad
    bg_y1 = bar_y + bar_h + pad
    cv2.rectangle(img, (bg_x0, bg_y0), (bg_x1, bg_y1), (30,30,30), -1)
    # 贴 colorbar
    img[bar_y:bar_y+bar_h, bar_x:bar_x+bar_w] = colored_bar

    # 边框
    cv2.rectangle(img, (bar_x, bar_y), (bar_x+bar_w, bar_y+bar_h), (220,220,220), 1, lineType=cv2.LINE_AA)

    # 画刻度：min / median / max
    if finite_errors:
        all_vals = np.array(list(finite_errors.values()), dtype=np.float32)
        low = np.percentile(all_vals, 1)
        mid = np.median(all_vals)
        high = np.percentile(all_vals, 99)

        def val_to_y(v):
            # 线性映射到 bar 位置（注意 gradient 从上 high 到下 low）
            t = (high - v) / (high - low + 1e-12)  # 0..1
            return int(bar_y + np.clip(t, 0, 1) * bar_h)

        # 小线条和文字
        for label, v in [("max", high), ("med", mid), ("min", low)]:
            y_pos = val_to_y(v)
            cv2.line(img, (bar_x - 5, y_pos), (bar_x, y_pos), (220,220,220), 1, lineType=cv2.LINE_AA)
            text = f"{v:.2f}"
            txt_w, txt_h = cv2.getTextSize(text, font, 0.4, 1)[0]
            cv2.putText(img, text, (bar_x - txt_w - 10, y_pos + txt_h//2), font, 0.4, (240,240,240), 1, lineType=cv2.LINE_AA)

        # 在 colorbar 上方写标题
        cv2.putText(img, "Error", (bar_x, bar_y - 10), font, 0.5, (240,240,240), 1, lineType=cv2.LINE_AA)
    else:
        cv2.putText(img, "No finite error", (bar_x, bar_y + bar_h//2), font, 0.5, (0,0,255), 1, lineType=cv2.LINE_AA)
...
def main():
    ...
    for txt_file in txt_files:
        ...
        for method_name, subfolder in methods.items():
            ...
            vis_img = draw_error_points_on_image_v2(image, xy_dict, error_t)
