...
def draw_colorbar_legend(heat_img, vmax_px, ticks_px=(0,1,2,3,5), size=(220, 14), margin=12, loc='br'):
    """
...
for method in methods:
    ...
    for i in range(start_idx, end_idx + 1):
        # 第1遍：遍历所有算法，计算原始误差并缓存，不画图
        cache = {}  # method -> dict
        for method in methods:
            ref_path  = os.path.join(ref_dir,   f"{i}_0.png")
            qry_path  = os.path.join("/mnt/sda/MapScape/query/estimation/mp4_compare/USA_seq5_foggy_gif/", method, f"{i}_0.png")
            ref_img = cv2.imread(ref_path)
            qry_img = cv2.imread(qry_path)
            if ref_img is None or qry_img is None:
                print(f"[跳过] 帧 {i} {method}: 缺图");  continue
            ...
            try:
                ...
                print(f"[跳过] 帧 {i} {method}: 位姿缺失 {e}");  continue

            # warp ref -> query（已实现）
            warp_ref, valid_mask = warp_ref_to_query(ref_img, ref_depth, ref_T, rcamera, qry_T, qcamera, stride=1)
            ...
            # 原始误差（都在 [0,1] 空间）
            e_l1_raw  = np.abs(g_q - g_w)                          # [0,1]
            ncc_raw   = ncc_map(g_q, g_w, win=11, sigma=1.5)       # [-1,1]
            e_ncc_raw = (1.0 - ncc_raw) * 0.5                      # [0,1]
            ssim_raw  = ssim_map(g_q, g_w, win=11, sigma=1.5)      # [-1,1]
            e_ssim_raw= 1.0 - (ssim_raw + 1.0) * 0.5               # [0,1]

            cache[method] = dict(
                qry_img=qry_img, warp_ref=warp_ref, valid=valid_mask,
                e_l1=e_l1_raw, e_ncc=e_ncc_raw, e_ssim=e_ssim_raw
            )

        if not cache:
            continue

        # 第2遍：对同一帧所有算法一起计算归一化范围
        l1_lo,   l1_hi   = group_percentile_range([cache[m]['e_l1']   for m in cache],
                                                [cache[m]['valid']  for m in cache],
                                                P_LOW, P_HIGH)
        ncc_lo,  ncc_hi  = group_percentile_range([cache[m]['e_ncc']  for m in cache],
                                                [cache[m]['valid']  for m in cache],
                                                P_LOW, P_HIGH)
        ssim_lo, ssim_hi = group_percentile_range([cache[m]['e_ssim'] for m in cache],
                                                [cache[m]['valid']  for m in cache],
                                                P_LOW, P_HIGH)

        # 第3遍：逐算法用“统一范围”出图（热力图用组norm，二值用原始阈值）
        for method, item in cache.items():
            vmask = item['valid']
            # 组归一化
            e_l1_n   = norm_to_01(item['e_l1'],   l1_lo,  l1_hi)
            e_ncc_n  = norm_to_01(item['e_ncc'],  ncc_lo, ncc_hi)
            e_ssim_n = norm_to_01(item['e_ssim'], ssim_lo, ssim_hi)

            heat_l1  = heatmap_with_contours(e_l1_n,   vmask, levels=CONTOUR_LEVELS)
            heat_ncc = heatmap_with_contours(e_ncc_n,  vmask, levels=CONTOUR_LEVELS)
            heat_ssi = heatmap_with_contours(e_ssim_n, vmask, levels=CONTOUR_LEVELS)

            # 颜色条（固定 0..1，表示“组归一化后的相对强弱”）
            heat_l1  = draw_colorbar_legend(heat_l1,  vmax_px=1.0, ticks_px=[0,0.25,0.5,0.75,1.0], size=(200,14), loc='br')
            heat_ncc = draw_colorbar_legend(heat_ncc, vmax_px=1.0, ticks_px=[0,0.25,0.5,0.75,1.0], size=(200,14), loc='br')
            heat_ssi = draw_colorbar_legend(heat_ssi, vmax_px=1.0, ticks_px=[0,0.25,0.5,0.75,1.0], size=(200,14), loc='br')

            # 二值红色高亮：用“原始 e_ssim”做统一阈值（语义不受归一化影响）
            red_highlight = overlay_binary_mask(
                cv2.resize(item['qry_img'], (qcamera[0], qcamera[1])),
                (item['e_ssim'] >= SSIM_ERR_THRESH).astype(np.float32),
                color=(0,0,255), alpha=0.5
            )

            base = os.path.join(output_path, f"{i}_{method}")
            cv2.imwrite(base + "_warpRef2Qry.png", item['warp_ref'])
            cv2.imwrite(base + "_heat_L1_gnorm.png",     heat_l1)
            cv2.imwrite(base + "_heat_NCC_gnorm.png",    heat_ncc)
            cv2.imwrite(base + "_heat_SSIM_gnorm.png",   heat_ssi)
            cv2.imwrite(base + f"_mask_SSIM_gt{SSIM_ERR_THRESH:.2f}.png", red_highlight)