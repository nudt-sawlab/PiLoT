...
#---
    ...
    # ECEF 空间中距离（单位：米）
    # errors_3d = np.linalg.norm(points2d_ref_valid - points2d_query, axis=1)
...
#----
    ...
    errors_2d = np.linalg.norm(points2d_query - points2d_ref, axis=1)
    ...
    errors_2d_visible = errors_2d[valid_mask]

    # === 归一化 3D 误差，用于箭头颜色 ===
    vmin = 0.0
    vmax = 10.0
    # norm_err = (errors_2d_visible - errors_2d_visible.min()) / (errors_2d_visible.ptp() + 1e-5)
    norm_err = np.clip(errors_2d_visible - vmin) / (vmax - vmin, 0.0, 10.0)
    ...
    for (x1, y1), (x2, y2), err_norm in zip(pts_query_visible, pts_ref_visible, norm_err):
        ...
        cv2.arrowedLine(frame_copy, pt1, pt2, color, 2, tipLength=0.25)

    # === 用 matplotlib 添加 colorbar 并保存 ===
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.imshow(cv2.cvtColor(frame_copy, cv2.COLOR_BGR2RGB))
    ax.axis('off')

    # colorbar 对应 3D 误差（米）
    sm = plt.cm.ScalarMappable(cmap='jet', norm=plt.Normalize(vmin=0.0, vmax=10.0))
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax)
    cbar.set_label("3D Distance Error (m)", fontsize=12)

    plt.tight_layout()
    plt.savefig(f"{output_path}/{i}_arrow3d.png", dpi=150)
    plt.close()
    print('-')
