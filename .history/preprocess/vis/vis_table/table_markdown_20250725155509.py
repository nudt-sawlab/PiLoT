...
def summarize_metrics(csv_path):
    ...
    for method in all_methods:
        ...
        for _, row in sub.iterrows():
            ...
            med = row['MedianError']
            std = row['StdError']
            est_errors = np.random.normal(loc=med, scale=std, size=50)
...
if __name__ == "__main__":
    csv_file = "/mnt/sda/MapScape/query/estimation/result_images/feicuiwan/feicuiwan.csv"
    df_summary = summarize_metrics(csv_file)
    df_summary.to_csv("/mnt/sda/MapScape/query/estimation/result_images/feicuiwan/summary_by_method.csv", index=False)
