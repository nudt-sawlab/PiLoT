...
for recall_name, recall_idx in recall_indices.items():
    ...
    for mean_recall, method_label, values, color in method_lines:
        alpha = get_alpha(mean_recall)
        ax.plot(angles, values, label=method_label, color=color, linewidth=2, zorder=2)
        ax.fill(angles, values, color=color, alpha=alpha, zorder=1)
    # 极坐标轴设置
    ax.set_thetagrids(np.degrees(angles[:-1]), weather_labels)
    ax.set_yticks([20, 40, 60, 80, 100])
    ax.grid(True, linestyle="--", linewidth=1, color="#CCCCCC")
    ax.spines['polar'].set_visible(False)
    ax.set_yticklabels(["20%", "40%", "60%", "80%", "100%"], fontsize=12, fontweight='bold', family='serif')
