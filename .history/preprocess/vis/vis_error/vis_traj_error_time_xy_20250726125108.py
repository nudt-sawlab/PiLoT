...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        # 注释文本靠右侧自动对齐，不超图
        # 在当前方法循环中添加
        text_x = frame_ids[-1] - 10  # ✅ 靠右但不出边
        text_y = med + 0.15          # ✅ 稍高于中位线本身

        ax.text(text_x, text_y,
                f"{methods_name[method]}: {med:.2f}°",   # ✅ 含算法名和中位误差
                color=color,
                fontsize=9,
                ha='right',
                va='bottom',
                alpha=0.95,
                zorder=100,
                bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="none", alpha=0.8))  # ✅ 加浅白背景防遮挡
    # === 图属性 ===
