...
seq_list = sorted([f for f in os.listdir(os.path.join(data_root, "GT_vis")) if f.endswith(".txt")])
...
for seq in seq_list:
    seq = "switzerland_seq7@8@cloudy@200.txt"
    ...
    for m_idx, method in enumerate(methods_label):
        ...
        for i in range(len(pos_err)):
            if 0 <= speed_idx[i] < len(speed_bins)-1 and 0 <= angle_idx[i] < len(speed_bins)-1:
            ...
            if 0 <= angle_idx[i] < len(angle_bins)-1 and 0 <= speed_idx[i] < len(speed_bins)-1:
...
plt.tight_layout()
plt.savefig(f"{outputs}/{seq}box_poserr_vs_speed.png", dpi=330)
...
plt.tight_layout()
plt.savefig(f"{outputs}/box_angerr_vs_angspeed.png", dpi=330)
