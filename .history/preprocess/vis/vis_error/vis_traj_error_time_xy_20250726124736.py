...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        # === 中位线 glow 底线（放在散点下方）
        ax.axhline(med, linestyle='-', color='white', linewidth=6, alpha=0.7, zorder=4)
        # === 主线（线宽中等，zorder略低于散点）
        ax.axhline(med, linestyle='-', color=dark_color, linewidth=2.2, alpha=0.9, zorder=5)
        # === 文本标注（靠图右边）
        ax.text(frame_ids[-1] + 10, med,
                f"{methods_name[method]}: {med:.2f}°",
                color=dark_color,
                fontsize=9,
                va='center',
                alpha=0.95,
