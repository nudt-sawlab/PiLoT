...
methods = {
    "FPVLoc": "#007F49",      # ✅ 深绿（GeoPixel）
    "Pixloc": "#86AED5",       # 柔和蓝
    "Render2loc": "#EF6C5D",   # 珊瑚橙（偏红，避免和黄混）
    "ORB@per30": "#C79ACD",    # 紫色
    "Render2loc@raft": "#B5C84A" # 亮金绿，区分橘粉 ✅
}
start_frame = 0  # ✅ 可调
end_frame = 900
...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        ax.scatter(frame_ids, pos_err_clipped,
               ...
               alpha=0.7,
               s=18,
               zorder=10 if method == "FPVLoc" else 5)  # ✅ GeoPixel置顶
        ...
        ax.axhline(med, linestyle='--', color=color, linewidth=1.2, alpha=0.4,
               zorder=1 if method != "FPVLoc" else 9)  # ✅ 线也靠上
