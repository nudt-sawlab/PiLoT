...
method_colors = [
    "#222222",  # GT - Black
    "#D55E00",  # FPVLoc/GeoPixel - Orange/Red
    "#009E73",  # PixLoc - Green
...
for seq in seq_list:
    ...
    for m_idx, method in enumerate(methods):
        ...
        pos_err = np.clip(pos_err, LOG_MIN_POSERR, POS_ERR_CLIP)
        ang_err = np.clip(ang_err, LOG_MIN_ANGERR, ANG_ERR_CLIP)
...
def grouped_violin_plot(ax, data_dict, methods, method_colors, bins, ylabel, CLIP, log_min, legend_loc):
    width = 0.12
...
grouped_violin_plot(ax1, pos_err_per_bin, methods, method_colors, speed_bins, "Position Error (m)", POS_ERR_CLIP, LOG_MIN_POSERR, "upper right")
plt.tight_layout()
...
grouped_violin_plot(ax2, ang_err_per_bin, methods, method_colors, angle_bins, "Angle Error (deg)", ANG_ERR_CLIP, LOG_MIN_ANGERR, "upper right")
plt.tight_layout()
plt.savefig(f"{outputs}/nature_violin_log_angerr_vs_angspeed.png", dpi=300)
plt.show()

print("✅ log纵坐标分组小提琴图已保存。")
