...
for seq in seq_list:
    ...
    for m_idx, method in enumerate(methods):
        ...
        est_frameidx = np.arange(len(gt_speed))
        est_xyz = np.ones_like(gt_xyz) * 99
        est_angles = np.ones_like(gt_angles)* 99
        tm = []
        for d, xyz in zip(est_data, est_xyz_list):
            ...
            est_frameidx[frame_idx] = frame_idx
        ...
        if 'ORB' in method:
            ...
            scale, R, t = umeyama_alignment(np.array(est_xyz[tm]), np.array(gt_xyz[tm]))
...
def nice_labels(bins, var_latex, unit):
    ...
    for i in range(len(bins)-1):
        ...
        # 最后一段左闭右闭
        if i < len(bins)-2:
            labels.append(rf"${left}\leq {var_latex} <{right}$ {unit}")
        else:
            labels.append(rf"${left}\leq {var_latex}\leq {right}$ {unit}")
    return labels
def grouped_violin_plot(ax, data_dict, methods, method_colors, bins, ylabel, log_min, legend_loc):
    ...
    sns.violinplot(
        ...
        scale="width", inner="quartile", cut=1, linewidth=1.4,
    ...
    if "Pos" in ylabel:
        xvar_latex, xunit = "v", r"\mathrm{m/frame}"
        ax.set_xticklabels(nice_labels(speed_bins, xvar_latex, xunit), fontsize=13)
        ax.set_xlabel(r"Speed $v$ ($\mathrm{m/frame}$) bin", fontsize=14)
        ax.set_ylabel(r"$\log_{10}(\Vert \Delta \mathbf{p} \Vert)$ (m)", fontsize=15)
        ax.set_title(r"Position Error Distribution across $v$ bins", fontsize=15)
    else:
        xvar_latex, xunit = r"\omega", r"\mathrm{deg/frame}"
        ax.set_xticklabels(nice_labels(angle_bins, xvar_latex, xunit), fontsize=13)
        ax.set_xlabel(r"Angular Speed $\omega$ ($\mathrm{deg/frame}$) bin", fontsize=14)
        ax.set_ylabel(r"$\log_{10}(\Delta \theta)$ (deg)", fontsize=15)
        ax.set_title(r"Angle Error Distribution across $\omega$ bins", fontsize=15)
