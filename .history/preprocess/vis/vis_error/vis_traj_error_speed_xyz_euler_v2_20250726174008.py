...
def grouped_box_plot(ax, data_dict, methods, method_colors, bins, ylabel, log_min, legend_loc):
    ...
    # Simplified labels to avoid LaTeX issues
    if "Pos" in ylabel:
        xvar_latex, xunit = "v", "m/frame"
        ax.set_xticklabels(nice_labels(bins, xvar_latex, xunit), fontsize=10)
        ax.set_xlabel(f"Speed ${xvar_latex}$ ({xunit})", fontsize=12)
        ax.set_ylabel(r"$\log_{10}(||\Delta \mathbf{p}||)$ (m)", fontsize=12)
        ax.set_title("Position Error vs. Speed", fontsize=14)
    else:
        xvar_latex, xunit = r"\omega", "deg/frame"
        ax.set_xticklabels(nice_labels(bins, xvar_latex, xunit), fontsize=10)
        ax.set_xlabel(f"Angular Speed ${xvar_latex}$ ({xunit})", fontsize=12)
        ax.set_ylabel(r"$\log_{10}(\Delta \theta)$ (deg)", fontsize=12)
        ax.set_title("Angle Error vs. Angular Speed", fontsize=14)
