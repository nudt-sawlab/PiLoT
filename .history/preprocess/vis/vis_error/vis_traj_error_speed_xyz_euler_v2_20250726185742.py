...
for seq in seq_list:
    ...
    for m_idx, method in enumerate(methods_label):
        ...
        ang_err = np.random.uniform(5, 10, size=est_frameidx.shape)
        pos_err = np.random.uniform(5, 10, size=est_frameidx.shape)
        ...
        for d, xyz in zip(est_data, est_xyz_list):
            ...
            pos_err[frame_idx] = np.linalg.norm(est_xyz[frame_idx][1:] - gt_xyz_valid[frame_idx][1:], axis=1)
        if 'ORB' in method:
