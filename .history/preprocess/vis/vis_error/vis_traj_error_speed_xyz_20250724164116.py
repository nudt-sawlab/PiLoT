...
for seq in seq_list:
    ...
    gt_speed = np.concatenate([[gt_speed[0]], gt_speed])  # 或 [np.nan] 替换0
    gt_angspeed = np.linalg.norm(gt_angles[1:] - gt_angles[:-1], axis=1)
    gt_angspeed = np.concatenate([[gt_angspeed[0]], gt_angspeed])
    # gt_v = np.linalg.norm(gt_xyz_valid[1:] - gt_xyz_valid[:-1], axis=1)
    # gt_av = np.linalg.norm(gt_angles_valid[1:] - gt_angles_valid[:-1], axis=1)
    for m_idx, method in enumerate(methods):
        ...
        pos_err = pos_err + np.random.uniform(-1e-4, 1e-4, size=pos_err.shape)
        ...
        if 'raft' in method:
            ...
            pos_err_sorted = sorted(pos_err)
            ang_err_sorted = sorted(ang_err)
            print('raft')
        if 'ORB' in method:
            print('ORB')
