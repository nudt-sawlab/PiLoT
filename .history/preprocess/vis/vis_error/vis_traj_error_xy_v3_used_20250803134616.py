...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        # 平滑轨迹（在 valid_mask 区域上做）
        if np.sum(valid_mask) > 5:
            rel_smooth = rel.copy()
            rel_smooth[valid_mask, 0] = gaussian_filter1d(rel[valid_mask, 0], sigma=2)
            rel_smooth[valid_mask, 1] = gaussian_filter1d(rel[valid_mask, 1], sigma=2)
        else:
            rel_smooth = rel
        # 创建图像
        ...
        ax.plot(rel_smooth[valid, 0], rel_smooth[valid, 1], color=color, linewidth=2.5)
