...
def plot_colored_trajectory(ax, traj, errors, cmap='viridis_r', linewidth=2.5, label=None):
    """
    绘制误差着色的轨迹曲线。
    traj: (N, 2) 相对轨迹坐标
    errors: (N,) 每帧误差值
    cmap: 颜色映射，从深色（误差小）到浅色（误差大）
    """
    ...
    lc.set_array(errors[:-1])  # 每段线用起点的误差着色
    lc.set_linewidth(linewidth)
    line = ax.add_collection(lc)

    # 用label加一个隐形点用于legend
    cmap_obj = plt.get_cmap(cmap)
    avg_err = np.nanmean(errors)
    color = cmap_obj((avg_err - np.nanmin(errors)) / (np.nanmax(errors) - np.nanmin(errors) + 1e-6))
...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        # 对应帧的误差值
        # 调用新函数画带误差着色的轨迹
        line = plot_colored_trajectory(ax_xy, rel, pos_err, cmap='viridis_r', label=methods_name[method])
