import numpy as np
from transform import WGS84_to_ECEF, ECEF_to_WGS84
...
all_orb_dict, orb_euler_dict = load_pose_dict("/mnt/sda/MapScape/query/estimation/result_images/ORB@per30/USA_seq5@8@cloudy@300-100@200_1.txt")
...
with open("/mnt/sda/MapScape/query/estimation/result_images/ORB@per30/USA_seq5@8@cloudy@300-100@200.txt", 'w') as f:
    for name, xyz in zip(all_orb_names, aligned_xyz, orb_euler_dict):
        xyz = ECEF_to_WGS84(xyz)
        f.write(f"{name} {xyz[0]} {xyz[1]} {xyz[2]} {orb_euler_dict[0]} {orb_euler_dict[1]} {orb_euler_dict[2]}\n")
