...
def plot_trajectory_with_gaps(ax, timestamps, traj, color, label=None, linewidth=2.2):
    x, y = traj[:, 0], traj[:, 1]
    ...
    for i in range(1, len(x) - 1):
        if isnan[i] and not isnan[i - 1] and not isnan[i + 1]:
            ax.plot([x[i - 1], x[i + 1]], [y[i - 1], y[i + 1]], color=color,
                    linestyle='dashed', linewidth=1.2, alpha=0.6)
...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        ax_alt.plot(full_timestamps[~large_error], aligned_xyz[~large_error, 2], color=color, linewidth=1.8)
        ax_alt.scatter(full_timestamps[large_error], aligned_xyz[large_error, 2], color=color, s=10, alpha=0.6)

        ax_pitch.plot(full_timestamps[~large_error], aligned_angles[~large_error, 0], color=color, linewidth=1.5)
        ax_pitch.scatter(full_timestamps[large_error], aligned_angles[large_error, 0], color=color, s=10, alpha=0.6)

        ax_yaw.plot(full_timestamps[~large_error], aligned_angles[~large_error, 1], color=color, linewidth=1.5)
