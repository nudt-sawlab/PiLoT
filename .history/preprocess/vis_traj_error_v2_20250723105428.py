...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        #================
        if 'GT' in method:
            gt_pts_cgcs = aligned_xyz
        if 'ORB' in method:
            orb_pts_cgcs = aligned_xyz
            scale, R, t = umeyama_alignment(orb_pts_cgcs[timestamps_est], gt_pts_cgcs[timestamps_est])
            orb_pts_cgcs = transform_points(orb_pts_cgcs, scale, R, t)
            aligned_xyz = orb_pts_cgcs
        #==========
        # 相对轨迹（NaN保留断线）
