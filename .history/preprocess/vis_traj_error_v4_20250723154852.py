...
def plot_with_virtual_lines(ax, x, y, color, large_error, label=None, linewidth=1.8):
    isnan = np.isnan(y)
    segments = []
    current_x, current_y = [], []

    for i in range(len(y)):
        if not isnan[i] and large_error[i] < 5:
    ...
    while i < len(y):
        if np.isnan(y[i]):
            ...
            i += 1
def plot_trajectory_with_gaps(ax, timestamps, traj, color, large_error, label=None, linewidth=2.2):
    x, y = traj[:, 0], traj[:, 1]
