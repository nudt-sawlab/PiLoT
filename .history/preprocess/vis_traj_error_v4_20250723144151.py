...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        plot_with_shaded_error(ax_alt, timestamps_est, poses_est[:, 2], np.abs(poses_est[:, 2] - poses_gt[:, 2]), method, color)
        plot_with_shaded_error(ax_pitch, timestamps_est, angles_est[:, 0], np.abs(angles_est[:, 0] - angles_gt[:, 0]), method, color)
        plot_with_shaded_error(ax_yaw, timestamps_est, angles_est[:, 1], np.abs(angles_est[:, 1] - angles_gt[:, 1]), method, color)
