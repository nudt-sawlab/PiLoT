...
def batch_traj_two_with_axes_tour(
    ...
    base_time="2025-01-01T00:00:00Z",
    flyto_alt=120.0,
    follow="est",                 # 相机跟随哪条轨迹："est" | "gt"
    axis_len_m=15.0,
    axis_width=3.0,
    tilt_from_pitch=True,
    yaw_add_90=True               # 你的数据里 yaw 需 +90°
):
    """
    同时播放 估计轨迹 与 GT 轨迹：两条 gx:Track + 累积轨迹线段 + 每帧坐标轴（头部）。
    双击 KML 内的 Tour 播放。
    """
    import simplekml, numpy as np, math
    from datetime import datetime, timedelta, timezone
    ...
    def parse_pose(parts):
        # parts: [name, lon, lat, alt, roll, pitch, yaw]
        _, lon_s, lat_s, alt_s, roll_s, pitch_s, yaw_s = parts[:7]
        lon = float(lon_s); lat = float(lat_s); alt = float(alt_s)
        roll = float(roll_s); pitch = float(pitch_s); yaw = float(yaw_s)
        return lon, lat, alt, roll, pitch, yaw
    ...
    if N_est == 0 or N_gt == 0:
        raise RuntimeError("估计或GT文件为空")

    # 按尾部跳过
    end_est = max(0, N_est - skip_tail)
    end_gt  = max(0, N_gt  - skip_tail)
    ...
    idxs_est = idxs_est[:n_frames]
    idxs_gt  = idxs_gt[:n_frames]
    ...
    # 颜色：EST（白轨，轴RGB），GT（青轨，轴半透明RGB）
    track_est.style.linestyle.color = simplekml.Color.rgb(255, 255, 255, 255)   # 白
    track_gt.style.linestyle.color  = simplekml.Color.rgb(0, 255, 255, 255)     # 青

    AX_R  = simplekml.Color.rgb(255,   0,   0, 255)
    AX_G  = simplekml.Color.rgb(  0, 255,   0, 255)
    AX_B  = simplekml.Color.rgb(  0, 128, 255, 255)
    AX_Rt = simplekml.Color.rgb(255,   0,   0, 180)  # GT 轴半透明
    ...
    for f in range(n_frames):
        ...
        parts_e = lines_est[i_est].split()
        parts_g = lines_gt[i_gt].split()

        lon_e, lat_e, alt_e, roll_e, pitch_e, yaw_e = parse_pose(parts_e)
        lon_g, lat_g, alt_g, roll_g, pitch_g, yaw_g = parse_pose(parts_g)
        ...
        # 收集 Track
        whens_est.append(when)
        coords_est.append((lon_e, lat_e, alt_e))

        whens_gt.append(when)
        coords_gt.append((lon_g, lat_g, alt_g))

        # EST 头部坐标轴（仅本帧显示）
        R_e = rot_zyx(yaw_e, pitch_e, roll_e)
        x_e = mv(R_e, [axis_len_m, 0, 0])
        y_e = mv(R_e, [0, axis_len_m, 0])
        z_e = mv(R_e, [0, 0, axis_len_m])
        X_e = enu_to_llh(lon_e, lat_e, alt_e, x_e[0], x_e[1], x_e[2])
        Y_e = enu_to_llh(lon_e, lat_e, alt_e, y_e[0], y_e[1], y_e[2])
        Z_e = enu_to_llh(lon_e, lat_e, alt_e, z_e[0], z_e[1], z_e[2])

        def add_axis(folder, name, color, p0, p1, begin, end, w=axis_width):
            ls = folder.newlinestring(name=name)
            ls.coords = [p0, p1]
            ls.altitudemode = simplekml.AltitudeMode.absolute
            ls.style.linestyle.color = color
            ls.style.linestyle.width = w
            ls.timespan.begin = begin
            ls.timespan.end   = end

        add_axis(folder_est, f"axisE_X_{i_est}", AX_R,  (lon_e, lat_e, alt_e), X_e, when, when_n)
        add_axis(folder_est, f"axisE_Y_{i_est}", AX_G,  (lon_e, lat_e, alt_e), Y_e, when, when_n)
        add_axis(folder_est, f"axisE_Z_{i_est}", AX_B,  (lon_e, lat_e, alt_e), Z_e, when, when_n)

        # GT 头部坐标轴（仅本帧显示，半透明）
        R_g = rot_zyx(yaw_g, pitch_g, roll_g)
        x_g = mv(R_g, [axis_len_m, 0, 0])
        y_g = mv(R_g, [0, axis_len_m, 0])
        z_g = mv(R_g, [0, 0, axis_len_m])
        X_g = enu_to_llh(lon_g, lat_g, alt_g, x_g[0], x_g[1], x_g[2])
        Y_g = enu_to_llh(lon_g, lat_g, alt_g, y_g[0], y_g[1], y_g[2])
        Z_g = enu_to_llh(lon_g, lat_g, alt_g, z_g[0], z_g[1], z_g[2])

        add_axis(folder_gt,  f"axisG_X_{i_gt}", AX_Rt, (lon_g, lat_g, alt_g), X_g, when, when_n)
        add_axis(folder_gt,  f"axisG_Y_{i_gt}", AX_Gt, (lon_g, lat_g, alt_g), Y_g, when, when_n)
        add_axis(folder_gt,  f"axisG_Z_{i_gt}", AX_Bt, (lon_g, lat_g, alt_g), Z_g, when, when_n)

        # 累积轨迹段（出现后不消失）
        def add_seg(folder, name, p0, p1, begin, end, color, width=3):
            seg = folder.newlinestring(name=name)
            seg.coords = [p0, p1]
            seg.altitudemode = simplekml.AltitudeMode.absolute
            seg.style.linestyle.color = color
            seg.style.linestyle.width = width
            seg.timespan.begin = begin
            seg.timespan.end   = end

        if prev_est is not None:
            add_seg(folder_est, f"trailE_{i_est-1}_{i_est}", prev_est, (lon_e, lat_e, alt_e),
                    when, final_end_time, simplekml.Color.rgb(255,255,255,255), 3)
        if prev_gt is not None:
            add_seg(folder_gt,  f"trailG_{i_gt-1}_{i_gt}", prev_gt,  (lon_g, lat_g, alt_g),
                    when, final_end_time, simplekml.Color.rgb(0,255,255,255), 3)

        prev_est = (lon_e, lat_e, alt_e)
        prev_gt  = (lon_g, lat_g, alt_g)

        # Tour：相机跟随（更丝滑：gxduration=1/fps，不 wait）
        cam_lon, cam_lat, cam_alt, cam_yaw, cam_pitch, cam_roll = (
            (lon_e, lat_e, alt_e, yaw_e, pitch_e, roll_e) if follow == "est"
            else (lon_g, lat_g, alt_g, yaw_g, pitch_g, roll_g)
        )
        flyto = playlist.newgxflyto(gxduration=1.0 / max(1, fps))
        tilt = float(max(0.0, min(90.0, 90.0 - cam_pitch))) if tilt_from_pitch else 75.0
        flyto.camera = simplekml.Camera(
            longitude=cam_lon, latitude=cam_lat, altitude=cam_alt + flyto_alt,
