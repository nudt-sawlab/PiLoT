...
def plot_uncertainty_band(ax, x, y, err, color):
    y_upper = y + err
    y_lower = y - err
    ax.fill_between(x, y_lower, y_upper, color=color, alpha=0.2)
def plot_with_shaded_error(ax, x, y, err, label, color):
...
for seq in seq_list:
    ...
    ax_poserr.plot(speed_timestamps, gt_speed_smooth, color='gray', linewidth=2.0, label='GT Speed', alpha=0.8)
    ax_angerr.plot(speed_timestamps, gt_ang_speed_smooth, color='gray', linewidth=2.0, label='GT Angular Speed', alpha=0.8)
    ...
    for method, color in methods.items():
        ...
        if 'GT' in method:
            gt_pts_cgcs = rel 
            ax_xy.plot(rel[:, 0], rel[:, 1], color=color, linewidth=2.2, label=method, alpha=0.6)
            ax_alt.plot(full_timestamps, aligned_xyz[:, 2], color=color, linewidth=2.2, label=method, alpha=0.6)
            ax_pitch.plot(full_timestamps, aligned_angles[:, 0], color=color, linewidth=2.2, label=method, alpha=0.6)
            ax_yaw.plot(full_timestamps, aligned_angles[:, 1], color=color, linewidth=2.2, label=method, alpha=0.6)
            continue
        ...
        large_error = pos_err > 5
        ax_xy.plot(rel[~large_error, 0], rel[~large_error, 1], color=color, linewidth=1.8, label=method, alpha=0.8)
        ax_xy.scatter(rel[large_error, 0], rel[large_error, 1], color=color, s=10, alpha=0.4)

        ax_alt.plot(full_timestamps[~large_error], aligned_xyz[~large_error, 2], color=color, linewidth=1.8, label=method, alpha=0.8)
        ax_alt.scatter(full_timestamps[large_error], aligned_xyz[large_error, 2], color=color, s=10, alpha=0.4)

        ax_pitch.plot(full_timestamps[~large_error], aligned_angles[~large_error, 0], color=color, linewidth=1.5, label=method, alpha=0.8)
        ax_pitch.scatter(full_timestamps[large_error], aligned_angles[large_error, 0], color=color, s=10, alpha=0.4)

        ax_yaw.plot(full_timestamps[~large_error], aligned_angles[~large_error, 1], color=color, linewidth=1.5, label=method, alpha=0.8)
        ax_yaw.scatter(full_timestamps[large_error], aligned_angles[large_error, 1], color=color, s=10, alpha=0.4)

        # 添加误差带
        # XY 平面误差带
        dx = poses_est[:, 0] - poses_gt[:, 0]
        dy = poses_est[:, 1] - poses_gt[:, 1]
        r = np.sqrt(dx ** 2 + dy ** 2)
        ax_xy.plot(poses_est[:, 0], poses_est[:, 1], color=color, label=method, alpha=0.9)
        ax_xy.scatter(poses_est[:, 0], poses_est[:, 1], s=10, c=color, alpha=0.3)

        # 海拔、角度添加误差带
        plot_with_shaded_error(ax_alt, full_timestamps, poses_est[:, 2], np.abs(poses_est[:, 2] - poses_gt[:, 2]), method, color)
        plot_with_shaded_error(ax_pitch, full_timestamps, angles_est[:, 0], np.abs(angles_est[:, 0] - angles_gt[:, 0]), method, color)
        plot_with_shaded_error(ax_yaw, full_timestamps, angles_est[:, 1], np.abs(angles_est[:, 1] - angles_gt[:, 1]), method, color)
