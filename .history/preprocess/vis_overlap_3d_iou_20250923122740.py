...
def voxel_iou_and_visualize(
    ...
    P1_w -= np.mean(P1_w, axis=0, keepdims=True)
    P2_w -= np.mean(P1_w, axis=0, keepdims=True)
...
iou3d, (mask_ref, vis_ref), (mask_qry, vis_qry) = voxel_iou_and_visualize(
    ...
    voxel_size=1.0,  # 体素边长(米)，0.5~2.0 视深度精度
