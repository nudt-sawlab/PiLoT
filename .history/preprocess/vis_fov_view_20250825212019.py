...
def batch_fov_visualization_with_error(
    ...
    sample_step=2,               # 与你原来的 idx % 2 对齐
    dist_forward=4.0, width=8.0, height=8.0,
    cm_vmin=0.0, cm_vmax=5.0,    # 颜色条范围（单位：米），按你的误差分布改
    face_alpha=90, line_alpha=200
):
    """
    从 txt_path 读取：filename lon lat alt roll pitch yaw
    从 error_txt 读取：每帧一个误差（与轨迹帧一一对应）。
    采样出图（每 sample_step 帧），并按误差着色。
    """
    # 读误差
    ...
    used_idx = 0   # 指向 errors 的索引（只在采样帧上递增）
    with open(txt_path, 'r') as f0:
        for idx, line in enumerate(f0):
            ...
            # 对应误差（若长度不够，用最后一个）
            if used_idx < len(errors):
                err = float(errors[used_idx])
            else:
                err = float(errors[-1])
            used_idx += 1
    ...
    kml.save(output_kml)
    print(f"[INFO] Colored KML saved to {output_kml}")
