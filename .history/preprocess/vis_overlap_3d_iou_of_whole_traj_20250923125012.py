...
# 先用 1/3、2/3 分位点
q1, q2 = np.quantile(ious, [1/3, 2/3])
# 若分布过于集中，回退到 25%/75%
if np.isclose(q1, q2, atol=1e-6):
    q1, q2 = np.quantile(ious, [0.25, 0.75])
# 若仍相等，再加微小扰动确保可分（不影响语义）
if np.isclose(q1, q2, atol=1e-12):
    eps = 1e-6
    q1, q2 = float(q1 - eps), float(q2 + eps)

bins = {"high": [], "mid": [], "low": []}
for (idx, d0, d1, v) in pair_infos:
    if v >= q2:
        bins["high"].append((idx, v))
    elif v >= q1:
        bins["mid"].append((idx, v))
    else:
        bins["low"].append((idx, v))

# ======= 汇总 =======
print("\nSummary (quantile-based bins):")
print(f"Total pairs: {len(pair_infos)}")
print(f"Thresholds: low < {q1:.4f} <= mid < {q2:.4f} <= high")
print(f"High (>= {q2:.4f}): {len(bins['high'])}")
print(f"Mid  [{q1:.4f}, {q2:.4f}): {len(bins['mid'])}")
print(f"Low  (< {q1:.4f}): {len(bins['low'])}")