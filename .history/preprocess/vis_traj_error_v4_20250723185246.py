...
def umeyama_alignment(src, dst):
    ...
    if np.linalg.det(R) < 0:
        U[:, -1] *= -1
        R = U @ Vt
    scale = np.trace(D) / (np.sum(src_c**2)/src.shape[0])
...
for seq in seq_list:
    ...
    for method,color in methods.items():
        ...
        # 方案一：偏移展示
        dx,dy = offset_dict.get(method,(0,0))
        rel[mask,0]+=dx; rel[mask,1]+=dy

        # ORB 对齐
        if 'ORB' in method:
            s,Rm,tm = umeyama_alignment(rel[ts[:300]], rel[ts[:300]])
            rel = s*(Rm@rel.T).T + tm
        ...
        large_p = pos_err>20; large_a=ang_err>5

        # 方案三：Marker 大小映射误差
        # 实线轨迹
        ax_xy.plot(rel[mask,0], rel[mask,1], color=color, linewidth=1.5, alpha=0.8, label=method)
        # 误差点以 marker size 表示
        sz = (pos_err/pos_err.max())*50 + 5
        ax_xy.scatter(rel[mask,0], rel[mask,1], s=sz[mask], color=color, alpha=0.6)
        ...
        while i<len(full_tstamps):
            if miss[i] or large_p[i]: st=i-1; 
                while i<len(full_tstamps) and (miss[i] or large_p[i]): i+=1
                en=i if i<len(full_tstamps) else i-1
                if 0<=st<len(full_tstamps) and en<len(full_tstamps):
                    ax_alt.plot([full_tstamps[st],full_tstamps[en]],[aligned_xyz[st,2],aligned_xyz[en,2]],
        ...
        miss_p = np.isnan(aligned_ang[:,0]); i=0
        while i<len(full_tstamps):
            if miss_p[i] or large_a[i]: st=i-1; 
                while i<len(full_tstamps) and (miss_p[i] or large_a[i]): i+=1
                en=i if i<len(full_tstamps) else i-1
                if 0<=st<len(full_tstamps) and en<len(full_tstamps):
                    ax_pitch.plot([full_tstamps[st],full_tstamps[en]],[aligned_ang[st,0],aligned_ang[en,0]],
                                  color=color,linestyle=(0,(10,6)),linewidth=2,alpha=0.8)
            else: i+=1
        ...
        miss_y=np.isnan(aligned_ang[:,1]); i=0
        while i<len(full_tstamps):
            if miss_y[i] or large_a[i]: st=i-1; 
                while i<len(full_tstamps) and (miss_y[i] or large_a[i]): i+=1
                en=i if i<len(full_timestamps) else i-1
                if 0<=st<len(full_timestamps) and en<len(full_timestamps):
                    ax_yaw.plot([full_tstamps[st],full_tstamps[en]],[aligned_ang[st,1],aligned_ang[en,1]],
                                color=color,linestyle=(0,(10,6)),linewidth=2,alpha=0.8)
            else: i+=1
    ...
    # 优化布局和图例
    ax_xy.set_title("Trajectory Comparison (offset & error-size markers)")
...
print("✅ 已完成方案一+方案三替换为方案一+方案二（基于 marker 大小的误差可视化），输出至 outputs/")
