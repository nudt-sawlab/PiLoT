...
def annotate_video_with_distance_and_error(video_path, pose_txt_path,
    ...
    # 每 N 帧对应一个 pose
    sampling_interval = int(frame_count / total_pose_len * speed_factor)
    ...
    while True:
        ...
        # 匹配 pose（考虑加速后每采样多少帧）
        pose_idx = min(int(frame_idx / sampling_interval), total_pose_len - 1)
        frame_name = positions[pose_idx][0]
        dist_m = distances[pose_idx]

        # 可视化：累计距离
        dist_text = f"Distance: {dist_m:.2f} m"
        cv2.putText(frame, dist_text, (margin, 40), font, font_scale,
                    (0, 0, 0), thickness_bg, cv2.LINE_AA)
        cv2.putText(frame, dist_text, (margin, 40), font, font_scale,
                    color_fg, thickness_fg, cv2.LINE_AA)

        # 可视化：轨迹误差（如果有）
        if pose_error_dict and frame_name in pose_error_dict:
            err_m = pose_error_dict[frame_name]
            err_text = f"Error: {err_m:.2f} m"
            cv2.putText(frame, err_text, (margin, 80), font, font_scale,
                        (0, 0, 0), thickness_bg, cv2.LINE_AA)
            cv2.putText(frame, err_text, (margin, 80), font, font_scale,
...
if __name__ == "__main__":
    video_path = "/home/ubuntu/Pictures/feicuwian_long_video_traj.mp4"
    pose_txt_path = "/mnt/sda/MapScape/query/estimation/result_images/FPVLoc/DJI_20250804192327_0002_.txt"
    output_path = "/home/ubuntu/Pictures/feicuwian_long_video_with_srt.mp4"
    speed_factor = 1.0
    fps = 30
