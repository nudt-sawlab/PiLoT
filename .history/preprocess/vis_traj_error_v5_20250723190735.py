...
for seq in seq_list:
    ...
    # 误差带可视化：每个点画误差圆
        xy_points = rel[valid_mask]
        xy_errs = pos_err[valid_mask]
        # 最大误差用于归一化
        max_radius = 10.0  # 最大可视化误差带半径（米），可根据实际场景调整
        for (x, y), e in zip(xy_points, xy_errs):
            radius = max(0.5, max_radius * (e / np.max(pos_err)))  # 保底最小圆
            circ = Circle((x, y), radius, color=color, alpha=0.17, linewidth=0)
