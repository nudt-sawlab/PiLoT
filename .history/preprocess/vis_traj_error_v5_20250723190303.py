...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        large_a = ang_err > 5
                        ...
                        # 方案三：XY轨迹误差色带（透明色带，带宽随误差增大）
        from matplotlib.collections import LineCollection
        xs = rel[mask, 0]
        ys = rel[mask, 1]
        errs = pos_err[mask]
        # 对 GT 绘制纯实线基线
        # 在主循环每个算法下，替换 XY 绘制部分为：

        if method == 'GT':
            ax_xy.plot(xs, ys, color=color, linewidth=2.5, label='GT')
        else:
            # 用逐段线宽渐变实现平滑色带
            for i in range(len(xs) - 1):
                w = 0.5 + 7.5 * ((errs[i] + errs[i+1]) / 2 / errs.max())  # 两帧误差均值
                ax_xy.plot(xs[i:i+2], ys[i:i+2], color=color, linewidth=w, alpha=0.6)
            # 自动调整坐标轴以显示色带
            ax_xy.relim()
