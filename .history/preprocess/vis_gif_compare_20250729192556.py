...
def make_pairwise_gif(folder1, folder2, output_dir, size=(480, 270), duration=500, loop=0):
    ...
    files2 = set(f for f in os.listdir(folder2) if f.endswith('.jpg'))

    common_files = sorted(files1.split('.')[0] & files2.split('.')[0])
    if not common_files:
        raise ValueError("两个文件夹中没有找到同名的 PNG 文件")
    ...
    for fname in common_files:
        ...
        # 保存 GIF 动图
        gif_path = os.path.join(output_dir, fname.replace('.png', '.gif'))
        img1_pil.save(
            gif_path,
            format='GIF',
            save_all=True,
...
if __name__ == "__main__":
    folder2 = "/mnt/sda/MapScape/query/estimation/mp4_compare/DJI_20250612174308_0001_V/GeoPixel"
    folder1 = "/mnt/sda/MapScape/query/estimation/mp4_compare/DJI_20250612174308_0001_V/GT_distorted"
    output_dir = "/mnt/sda/MapScape/query/estimation/mp4_compare/DJI_20250612174308_0001_V/gif"
