...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        ax_xy.plot(rel[~nan_error, 0], rel[~nan_error, 1], linestyle = 'dashed', color=color, linewidth=1.8, alpha = 0.4)
        xy_err = np.linalg.norm(aligned_xyz[:, :2] - poses_gt[:, :2], axis=1)
        xy_err[np.isnan(xy_err)] = 50
        plot_with_shaded_error(ax_xy, full_timestamps, xy_err, np.abs(xy_err), method, color)
        ...
        height_err = np.array(aligned_xyz[:, 2] - poses_gt[:, 2])
        height_err[np.isnan(height_err)] = 50
        plot_with_shaded_error(ax_alt, full_timestamps, height_err, np.abs(height_err), method, color)
        pitch_err = np.array(aligned_angles[:, 0] - angles_gt[:, 0])
        pitch_err[np.isnan(pitch_err)] = 10
        yaw_err = np.array(aligned_angles[:, 1] - angles_gt[:, 1])
        yaw_err[np.isnan(yaw_err)] = 10
        plot_with_shaded_error(ax_pitch, full_timestamps, aligned_angles[:, 0], np.abs(pitch_err), method, color)
        plot_with_shaded_error(ax_yaw, full_timestamps, aligned_angles[:, 1], np.abs(yaw_err), method, color)
