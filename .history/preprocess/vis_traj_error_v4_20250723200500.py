...
def plot_with_virtual_lines(ax, x, y, color, large_error=None,label=None, linewidth=1.8,
    ...
    for i, (xs, ys) in enumerate(segments):
        if color != 'black':
            ax.plot(xs, ys,  linewidth=2.0, alpha=0.7, color=color,  linewidth=linewidth,  label=label if i == 0 else None)
        else:   
...
for seq in seq_list:
    ...
    ax_yaw.scatter(full_timestamps, angles_gt[:, 1], color='black', s=30, alpha=0.5, label='GT')
    
    ax_yaw.set_ylim(185, 186)
    ax_yaw.set_xlim(182, 192)
    ax_yaw.yaxis.set_major_locator(mticker.MultipleLocator(0.2))
    ax_yaw.xaxis.set_major_locator(mticker.MultipleLocator(1))
    ax_yaw.grid(True)
    ax_yaw.legend()
