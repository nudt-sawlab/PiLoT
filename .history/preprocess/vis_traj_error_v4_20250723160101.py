    ...
    for i in range(len(y)):
        if not isnan[i]:
            ...
            if large_error is not None:
                current_err.append(large_error[i])
            else:
                current_err.append(False)
        # else:
        #     if len(current_x) > 1:
        #         # 检查当前段是否误差都在阈值内
        #         if large_error is None or max(current_err) < error_thresh:
        #             segments.append((current_x, current_y))
        #     current_x, current_y, current_err = [], [], []

    if len(current_x) > 1:
        if large_error is None or current_err is False:
            segments.append((current_x, current_y))
