...
for seq in seq_list:
    ...
    for method, color in methods.items():
        ...
        # --------- 计算 GT 飞行速度与角速度 ----------
    gt_speed = np.linalg.norm(np.diff(poses_gt, axis=0), axis=1)  # m/s
    gt_angular_speed = np.linalg.norm(np.diff(angles_gt, axis=0), axis=1)  # deg/s

    # 时间戳对齐（少一帧）
    speed_timestamps = full_timestamps[1:]

    # 速度曲线（GT基准线，用折线）
    ax_poserr.plot(speed_timestamps, gt_speed, color='black', linewidth=1.5, label='GT Speed (m/s)')
    ax_angerr.plot(speed_timestamps, gt_angular_speed, color='black', linewidth=1.5, label='GT Angular Speed (°/s)')
    ...
    for method, color in methods.items():
        ...
        # 使用 scatter 显示
        ax_poserr.scatter(full_timestamps, pos_err, color=color, label=method, s=8, alpha=0.6)
        ax_angerr.scatter(full_timestamps, ang_err, color=color, label=method, s=8, alpha=0.6)
    # 图表设置
    ...
    ax_poserr.grid(True)
    ax_poserr.set_ylim(0, 5)  # 限制最大位置误差
    ax_poserr.legend()
    ...
    ax_angerr.grid(True)
    ax_angerr.set_ylim(0, 5)  # 限制最大角度误差
    ax_angerr.legend()
