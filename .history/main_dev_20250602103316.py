...
class DualThreadTask:
    ...
    def localization_thread(self):
        with open(self.estimated_pose, "w") as f:
            ...
            # while not self.stop_event.is_set():
                self.loc_event.wait()  # 等待渲染线程的参考图准备好
                self.loc_event.clear()  # 清除事件，准备下一次等待
                if self.T_render and self.loc_flag == self.render_flag + 1:
                    # 计算并输出第n帧位姿解算结果
                    if self.loc_flag < len(self.img_list):
                        ...
                        if 'origin' in ret.keys():
                            ...
                            # self.last_frame_info['refine_conf']['origin'] = ret['origin']
                            self.last_frame_info['euler_angles'], self.last_frame_info['translation'] = ret['euler_angles'], ret['translation']
                        self.loc_flag += 1
                        while self.render_flag == self.loc_flag - 2: time.sleep(0.01)   # loc线程提前执行完
                        self.render_event.set()
                    else:
                        self.render_event.clear()
                        self.loc_event.clear()
                        self.stop_flag = True

                        
                # time.sleep(0.)   # 模拟任务的延迟

    def rendering_thread(self):
        while not self.stop_flag:
            self.render_event.wait()  # 等待定位线程的位姿准备好
            self.render_event.clear()  # 清除事件，准备下一次等待
            if (self.render_flag == self.loc_flag - 1 or self.render_flag == -2):  # 判断定位线程是否已经有数据
                euler_angles, translation = self.euler_angles, self.translation
                # 渲染RGB图与Depth图
                ...
                while (not self.stop_flag) & self.render_flag == self.loc_flag: time.sleep(0.01)   # render线程提前执行完
                self.loc_event.set()
            # time.sleep(0.5)  # 模拟任务的延迟
    ...
    def back_project(self, depth_frame, euler_angles, translation, num_samples = 500):
        # 
        ...
        Points_3D_ECEF, T_render_in_ECEF_w2c_modified, T_initial_pose_candidates, dd = get_3D_samples_v3(points2d, depth_frame, T_render_in_ECEF_c2w, self.render_camera,  euler_angles, translation, origin = self.origin, num_init_pose=self.num_init_pose,mul = self.mul)
        if dd is not None:
            self.dd = dd
    ...
    def input(self):
        ...
        self.render_event.set()
        while self.loc_flag < len(self.img_list):
...
if __name__ == "__main__":
    ...
    config_file = '/home/liuxy24/code/FPV_dev_quat_long/FPV-Test-quat-512/configs/USA_seq8@8@sunny@500@Unet_fusion.yaml'
