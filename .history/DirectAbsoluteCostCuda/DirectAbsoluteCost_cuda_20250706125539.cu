...
__global__ void fused_debug_all_kernel(
    ...
    float *__restrict__ weight_loss_out // [B * N * P]
)
{
...
#pragma unroll
    ...
    if (Zr <= 1e-4f)
    {
        ...
        weight_loss_out[(b * N + n) * P + p] = 0.0f;
        return;
    }
...
#pragma unroll
    ...
    weight_loss_out[(b * N + n) * P + p] = unc * loss_val;
...
)
{
    ...
    torch::Tensor weight_loss = torch::zeros({B, N, P}, opts);
    ...
    auto w_nz_mask = weight_loss.ne(0.0f); // 布尔张量，[B, N, P]，true 表示对应位置 loss ≠ 0
    auto w_nz_count = w_nz_mask.sum(-1);   // 在 P 维度上求和，得到 [B, N]，表示每个 (b,n) 下非零元素的个数
    // 为了避免除以零的情况，如果某些 (b,n) 一行里 loss 全为 0，就让计数至少为 1
    w_nz_count = w_nz_count.clamp_min(1);                  // [B, N]，把所有值 < 1 的先设为 1
    auto w_loss_sum = weight_loss.sum(-1);                 // [B, N]
    auto w_nz_count_f = w_nz_count.to(w_loss_sum.dtype()); // [B, N], float32
    auto w_loss_avg = w_loss_sum / w_nz_count_f;           // [B, N]
    return std::make_tuple(
        -grad, Hess, loss_avg, loss, w_loss_avg, weight_loss);
}
