...
def generate_rotvecs_cuda_sym3d(
    ...
    yaw   = Y_flat + base_yaw
    # 拼成 (N,3)
    print(torch.stack((P_flat, R_flat, Y_flat), dim=1))
...
def get_3D_samples_v3(mkpts_r, depth_mat, T_c2w, camera, euler_angles, translation, last_frame_info = {}, origin = None, device = 'cuda', num_init_pose = 32, T_query_in_ECEF_c2w = None, mul = None):
    '''
    1. 欧拉角转R， 经纬高转xyz
    2. preprocess_param， [:3, 1] = -[:3, 1], [:3, 2] = - [:3, 2]
    3. * mul
...
def generate_render_camera(camera):
    w, h, cx, cy, sensor_width, sensor_height, fmm_x, fmm_y = camera
    fx = w / sensor_width * fmm_x
    fy = h / sensor_height * fmm_y
    camera_param = {
    'model': 'PINHOLE',
    'width': w,
    'height': h,
