...
class DualThreadTask:
    ...
    def back_project(self, depth_frame, euler_angles, translation, num_samples = 200, device = 'cuda'):
        ...
        T_render_in_ECEF_c2w = torch.tensor(T_render_in_ECEF_c2w).to(device)
        self.render_camera = self.render_camera.to(device)
        self.origin = torch.tensor(self.origin).to(device)
        Points_3D_ECEF1, T_render_in_ECEF_w2c_modified1, T_initial_pose_candidates1, dd1 = get_3D_samples_v3(points2d, depth_frame, T_render_in_ECEF_c2w, self.render_camera,  euler_angles, translation, origin = self.origin, num_init_pose=self.num_init_pose,mul = self.mul, device=device)
        if dd is not None:
            self.dd = dd
