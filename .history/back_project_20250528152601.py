...
render_camera_osg = np.array(render_config['render_camera'])
render_camera_osg = render_camera_osg / 1
def render_scene( euler_angles, translation):
    for _ in range(20):
        renderer.update_pose(translation, euler_angles, ref_camera = render_camera_osg)
        color_image = renderer.get_color_image()
        depth_image = renderer.get_depth_image()
    return depth_image
for i in range(len(pose_dict)):
    ...
    end_time2 = time.time()
    print('--depth2: ', end_time2 - start_time)
    # depth_frame[0,0] = i
