...
class DualProcessTask:        
    def __init__(self, config, init_euler = None, init_trans = None, name = None):
        ...
        self.gt_pose = os.path.join(folder_path, 'srt', dataset_name +'.txt') #
    ...
    def localization_worker(self):
        ...
        for idx, (img_path, img_tensor) in enumerate(zip(self.img_list, self.query_list)):
            ...
            # 2) 反投影
            # t1 = time.time()
            P3d, T_w2c_mod, T_init, dd = self.back_project(depth, render_euler, render_trans)
            # t2 = time.time()
            # print('反投影耗时：', t2-t1)
            t0 = time.time()
            # 2) 调用定位
...
if __name__ == "__main__":
    ...
    # filname = None
    # name = None
    # config_file = "/home/ubuntu/Documents/code/github/FPV/FPV-Test-512-cuda/configs/feicuiwan_m4t.yaml"
