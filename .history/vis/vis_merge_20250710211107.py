...
for idx, (dset, step_idx, title) in enumerate(iters, start=1):
    ...
    pitch_jitter = np.asarray(pitch_jitter)
    yaw_jitter   = np.asarray(yaw_jitter)
    pitch_jitter[1] = pitch_jitter[0]
    yaw_jitter[1] = yaw_jitter[0]
    ...
    if idx == 3:
        opt_pitch, opt_yaw = p_sorted[0], y_sorted[0]
        ...
        arrow = FancyArrowPatch(
            ...
            connectionstyle="arc3,rad=0.25",         # 曲率，正值向左弯，负值向右弯
            arrowstyle='-|>,head_length=10,head_width=6',  # 线头和尖角
            color='#555555',                         # 深灰色
            linewidth=2.0,
            alpha=0.8,
            mutation_scale=15                       # 缩放箭头整体大小
        )
        ax.add_patch(arrow)
    # ——— 轮廓线 ———
