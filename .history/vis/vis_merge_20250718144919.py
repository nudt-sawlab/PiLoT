...
axes = axes.flatten()
x_min, x_max = -10, 20     # Pitch 范围，按需修改
y_min, y_max = -35, -5     # Yaw   范围，按需修改
for ax in axes:
...
for idx, (dset, step_idx, title) in enumerate(iters, start=1):
    ...
    if idx == 3:
        eulers = eulers[0:6].copy()
        losses = losses[0:6].copy()
        # print(eulers)
        # eulers[1] = eulers[0]
        # losses[1] = losses[0]
    pitch = eulers[:, 0]
    ...
    if idx ==3:
        y_sorted = y_sorted[:16]
        l_sorted = l_sorted[:16]
        p_sorted = p_sorted[:16]
    # ——— 优化点云 ———
    sc = ax.scatter(p_sorted, y_sorted, c=l_sorted, cmap='viridis',
                    s=40, alpha=0.7, edgecolors='none')
    # if idx == 3:
    #     opt_pitch, opt_yaw = p_sorted, y_sorted
    #     prior_pitch, prior_yaw = prior_euler[0], prior_euler[2]

        # # ——— 用 FancyArrowPatch 画一条带弧度的高级箭头 ———
        # arrow = FancyArrowPatch(
        #     posA=(prior_pitch, prior_yaw),
        #     posB=(opt_pitch,   opt_yaw),
        #     connectionstyle="arc3,rad=0.2",                  # 轻微弧度
        #     arrowstyle="Simple,head_length=6,head_width=3,tail_width=1",  
        #     color="dimgray",
        #     linewidth=1.5,
        #     alpha=0.8,
        #     mutation_scale=8                                # 缩小整体
        # )
        # ax.add_patch(arrow)
    # ——— 轮廓线 ———
