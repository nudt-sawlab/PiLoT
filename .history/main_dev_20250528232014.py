...
class DualProcessTask:          # ← 名字随意
    ...
    #             color = renderer.get_color_image()
    #             depth = renderer.get_depth_image()
    #             # renderer.save_color_image(os.path.join(self.outputs, 'init.png'))
    ...
    def back_project(self, depth_frame, euler_angles, translation, num_samples = 200, device = 'cuda'):
        ...
        Points_3D_ECEF, T_render_in_ECEF_w2c_modified, T_initial_pose_candidates, dd = get_3D_samples_v2(points2d, depth, T_render_in_ECEF_c2w, self.render_camera,  euler_angles, translation, origin = self.origin, num_init_pose=self.num_init_pose,mul = self.mul)
        Points_3D_ECEF1, T_render_in_ECEF_w2c_modified1, T_initial_pose_candidates1, dd1 = get_3D_samples_v3(points2d, depth, T_render_in_ECEF_c2w, self.render_camera,  euler_angles, translation, origin = self.origin, num_init_pose=self.num_init_pose,mul = self.mul)
        # if dd is not None:
        #     self.dd = 
        return Points_3D_ECEF, T_render_in_ECEF_w2c_modified, T_initial_pose_candidates, dd
