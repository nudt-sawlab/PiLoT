...
class DualThreadTask:
    ...
    def render_scene(self, euler_angles, translation):
        ...
        for _ in range(20):
            ...
            end_time = time.time()
            print('update pose time: ', end_time - start_time)
            start_time = time.time()
            color_image = self.renderer.get_color_image()
            end_time = time.time()
            print('color time: ', end_time - start_time)
            start_time = time.time()
            depth_image = self.renderer.get_depth_image()
            end_time = time.time()
            print('depth time: ', end_time - start_time)
        end_time1 = time.time()
        print('---render time: ', end_time1 - start_time1)
        depth1 = np.load('/home/ubuntu/Documents/code/github/FPV/FPV-Test-512-cuda/outputs/Mapsape/switzerland_seq4@8@sunny@500@512/0_0.npy')
